...
- **Default for local day-to-day dev/testing: connect directly to the real local PostgreSQL 18 install, not `shaneapp://executeSql`.** The `DATABASE_URL` env var in `.env.local` is a genuine, directly-reachable local Postgres connection string: `postgresql://postgres:<password>@localhost:5432/shanemccawmsp` (host `localhost`, port `5432`, db `shanemccawmsp`; the real password is in `.env.local` itself, not repeated here) — the same database the local dev api-server itself reads/writes. While building, agents should use it directly (`node scripts/db/psql.mjs -c "..."`, a one-off script, etc.) for reads to confirm state and for writes/`ALTER`/`UPDATE`/`INSERT` that are a normal, reversible part of the task — faster and with zero indirection through BuildConsole. Report the real result honestly, the same way test pass/fail is reported. Don't claim something is verified against live data unless it actually was queried.
- **Never hand-roll a bare `psql "$DATABASE_URL"` call — use `node scripts/db/psql.mjs` instead (Git #4526).** A build agent's Bash tool shell does not have `DATABASE_URL` exported (and should not — sourcing `.env.local` risks leaking `MT_APP_CERT_PRIVATE_KEY`, see the env-local-bash-source-leaks-pem-key memory), so `psql "$DATABASE_URL"` with an empty/unset var silently falls back to psql's default connection params and blocks on an interactive password prompt written directly to the controlling terminal — invisible to stdout/stderr capture, and it hangs forever rather than erroring. Confirmed live: 6 of the last 60 build-queue sessions ended a turn with exactly this hang, auto-backgrounded, holding the queue slot. `scripts/db/psql.mjs` closes this structurally: it loads `DATABASE_URL` safely (a single regex line out of `.env.local`, not a full `source`), always passes `-w` (never prompt), and sets a bounded `PGCONNECT_TIMEOUT` — any real connection failure now surfaces as a fast, capturable error instead of a silent hang. Any argv after the script path passes straight through to real `psql` (e.g. `node scripts/db/psql.mjs -c "\d some_table"`).
...
- **Never "live-verify" a destructive script or its UI wrapper by actually running its real destructive mode against the shared local dev DB (Git #4423).** The same live-consented mccawsoft2 tenant (a real M365 admin consent + real Stripe signup, walked through by hand) was deleted **six times in one day** — not by a cron job or a test teardown, but by five separate build sessions (#4393, #4415, #4417, #4436, #4437) each running `scripts/db/reset-dev-database.mjs --yes`, or typing a BuildConsole Command Center gate's real confirm phrase, as their own proof that the script/gate they'd just written or fixed actually worked. `reset-dev-database.mjs` now refuses its entire run when a tenant is flagged `reset_protected` (see `scripts/db/README.md`), but that only shields the one tenant someone remembered to flag — it is not a license to keep testing destructive scripts for real against shared state. Verify a destructive script or gate with `--dry-run` output, a throwaway/synthetic MSP that is not `is_direct_business = true`, or a standalone harness against a disposable copy — never by actually committing the real op against the shared database everyone's live walkthroughs and concurrent sessions depend on. This is the same "destructive changes go to Shane to run himself" rule two lines up; #4423 is the concrete cost of routing around it via "verification."
