...
function scRingColor(s: number) { return s >= 70 ? "#22c55e" : s >= 40 ? "#f59e0b" : "#ef4444"; }
function scTopBar(s: number)    { return s >= 70 ? "bg-green-500" : s >= 40 ? "bg-amber-400" : "bg-red-500"; }
function scStatusLabel(s: number) { return s >= 70 ? "Healthy" : s >= 40 ? "Attention" : "Critical"; }
function scStatusBadge(s: number) {
  return s >= 70
...
function ScoreRingCard({ score, size = 72 }: { score: number; size?: number }) {
  const r = (size / 2) - 7;
  ...
  return (
    <div className="relative flex-shrink-0" style={{ width: size, height: size }}>
      <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`} className="-rotate-90">
        ...
        <circle cx={cx} cy={cx} r={r} fill="none" stroke={scRingColor(score)} strokeWidth="6" strokeLinecap="round"
  ...
  return (
    <div className="relative flex-shrink-0" style={{ width: size, height: size }}>
      <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`} className="-rotate-90">
        ...
        <circle cx={cx} cy={cx} r={r} fill="none" stroke={scRingColor(score)} strokeWidth="7" strokeLinecap="round"
          strokeDasharray={`${(score / 100) * circ} ${circ}`} />
      </svg>
...
export default function ClientProjectDashboard() {
  ...
  useEffect(() => {
    ...
    fetchWithAuth("/api/portal/app-registration")
      ...
      .then((d: { status?: string } | null) => {
        setAppRegStatus(d?.status ?? "pending");
      })
      .catch(() => null);
  ...
  // True when app registration has not yet been submitted/verified — drives the synthetic Kanban card
  const showAppRegKanbanCard = appRegStatus !== null && appRegStatus !== "submitted" && appRegStatus !== "verified";
      ...
      <div className="max-w-5xl mx-auto px-4 pb-16">
        ...
        {appRegStatus !== null && appRegStatus !== "submitted" && appRegStatus !== "verified" && (
          <div className="mb-4 bg-[#0A2540] border-l-4 border-[#0078D4] rounded-xl p-4 flex flex-col sm:flex-row sm:items-center gap-3">
            <div className="flex items-start gap-3 flex-1 min-w-0">
              <div className="w-9 h-9 bg-[#0078D4]/20 rounded-xl flex items-center justify-center flex-shrink-0 mt-0.5">
            ...
            <div className="flex-shrink-0">
              <Link href="/portal/automation-setup" className="block text-xs font-bold text-[#0A2540] bg-[#0078D4] hover:bg-[#0078D4]/90 px-5 py-2.5 rounded-lg transition-colors whitespace-nowrap">
                Set Up Now →
              </Link>
            </div>
          </div>
                ...
                <div className="bg-[#0A2540] px-6 py-5">
                  <div className="flex items-center justify-between gap-4 flex-wrap">
                    <div className="flex items-center gap-4">
                      <OverallHealthRing score={overall} />
                      <div>
                        <p className="text-[10px] font-bold tracking-[0.2em] uppercase text-white/40">Mission Status</p>
                        <h2 className="text-xl font-black text-white tracking-tight leading-tight">M365 Environment Health</h2>
                        <div className="flex items-center gap-2 mt-1.5 flex-wrap">
                          <span className={`inline-flex items-center gap-1.5 text-[10px] font-bold px-2.5 py-1 rounded-md border ${scStatusBadge(overall)}`}>
                            <span className="w-1.5 h-1.5 rounded-full" style={{ backgroundColor: scRingColor(overall) }} />
                            {scStatusLabel(overall)}
                          </span>
                        ...
                        <Link key={key} href={`/portal/m365-profile?tab=${tab}`}>
                          <div className="bg-[#0A2540] hover:bg-[#0e2f50] border border-white/5 hover:border-[#0078D4]/40 rounded-xl overflow-hidden cursor-pointer transition-all group hover:shadow-xl hover:shadow-black/30">
                            ...
                            <div className="p-4 flex flex-col items-center gap-3">
                              <ScoreRingCard score={current} size={72} />
                              <p className="text-[11px] font-bold text-white/80 text-center leading-snug uppercase tracking-wide">{label}</p>
        ...
        {detailLoading ? (
          ...
          <div className="space-y-6">
            ...
            {((tasks && tasks.length > 0) || showAppRegKanbanCard) && (
              <div className="bg-white border border-border rounded-2xl p-5" ref={taskBoardRef}>
                ...
                <div className="grid grid-cols-4 gap-2 mb-5">
                  {([
                    ...
                    { label: "Waiting on You", count: taskCounts.waiting + (showAppRegKanbanCard ? 1 : 0), color: "text-yellow-600", bg: "bg-yellow-50 border-yellow-200", scroll: true },
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-3">
                  {(["backlog", "in_progress", "waiting_on_customer", "completed"] as const).map(col => {
                    ...
                    const colTasks = allColTasks.slice(0, col === "waiting_on_customer" && showAppRegKanbanCard ? 4 : 5);
                    const colCfg = {
                        ...
                        {(col !== "completed" || !completedCollapsed) && <div className="px-3 pb-3 space-y-2">
                          ...
                          {col === "waiting_on_customer" && showAppRegKanbanCard && (
                            <Link href="/portal/automation-setup">
                              <div className="w-full text-left bg-amber-50 rounded-lg border border-amber-300 p-2.5 hover:border-amber-400 hover:shadow-sm transition-all cursor-pointer">
                                <div className="flex items-start gap-2">
                                  ...
                                  <p className="text-xs font-semibold text-amber-900 line-clamp-2 leading-snug">Connect Your Azure Automation</p>
                                </div>
                                <p className="text-[10px] text-amber-700 mt-1.5 leading-snug">Shane needs access to your Azure tenant to run automation scripts. Tap to complete setup.</p>
                                <div className="flex items-center gap-1 mt-2">
                                  <span className="text-[9px] font-bold uppercase tracking-wide px-1.5 py-0.5 rounded bg-amber-200 text-amber-800">Action required</span>
                                </div>
                              </div>
                            </Link>
                          )}
                          {col === "waiting_on_customer" && showAppRegKanbanCard && colTasks.length === 0 && allColTasks.length === 0 ? null :
                          (!showAppRegKanbanCard || col !== "waiting_on_customer") && colTasks.length === 0 ? (
