...
export default function PortalAppRegistration() {
  ...
  async function handleRecheck() {
    ...
    try {
      const res = await fetchWithAuth("/api/portal/app-registration/recheck", { method: "POST" });
      if (!res.ok) {
        const err = await res.json() as { error?: string };
        setRecheckError(err.error ?? "Re-check failed. Please try again.");
        return;
      }
      const updated = await res.json() as AppRegRecord;
      setRecord(updated);
    } catch {
      setRecheckError("Network error. Please check your connection and try again.");
    } finally {
      setRechecking(false);
    }
  }
        ...
        {!loading && record?.permissionCheck && (
          <div className="space-y-3">
            ...
            <div className="flex items-center justify-between flex-wrap gap-2">
              <div>
                {(() => {
                  const { missing = [], granted = [] } = record.permissionCheck;
                  const total = missing.length + granted.length + (record.permissionCheck.unverifiable?.length ?? 0);
                  if (missing.length === 0) {
                    return (
                      <p className="text-sm font-semibold text-green-700">
                        ✓ All {total} permission{total === 1 ? "" : "s"} granted
                      </p>
                    );
                  }
                  return (
              ...
              {record.status === "verified" && !updateMode && (
                <button
                  type="button"
                  onClick={handleRecheck}
                  disabled={rechecking}
                  className="inline-flex items-center gap-1.5 text-xs font-semibold text-[#0078D4] hover:text-[#0078D4]/80 border border-[#0078D4]/30 hover:border-[#0078D4]/60 bg-white hover:bg-[#0078D4]/5 px-3 py-1.5 rounded-lg transition-colors disabled:opacity-60 disabled:cursor-not-allowed"
                >
                  {rechecking ? (
                    <>
                      <span className="w-3.5 h-3.5 border-2 border-[#0078D4]/40 border-t-[#0078D4] rounded-full animate-spin" />
                      Checking…
                    </>
                  ) : (
                    <>
