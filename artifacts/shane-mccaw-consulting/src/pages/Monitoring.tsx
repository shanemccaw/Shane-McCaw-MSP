  ...
  return (
    ...
    >
      ...
      {contactUs ? (
        <div className="mb-4">
          <p
            className={`text-2xl font-extrabold ${hl ? "text-white" : "text-[#0A2540]"}`}
          >
            Custom pricing
          </p>
          <p
            className={`text-sm mt-0.5 ${hl ? "text-white/50" : "text-muted-foreground"}`}
          >
        ...
        <div className="mb-4">
          {totalDollars !== null ? (
            <>
              <p className="text-[#0078D4] text-3xl font-extrabold">
              ...
              <p
                className={`text-xs mt-0.5 ${hl ? "text-white/40" : "text-muted-foreground"}`}
              >
                {tier.price ? fmtDollars(parseFloat(tier.price)) : "—"} per seat
                · {seats} seats
              </p>
            </>
          ) : (
            <p
              className={`text-2xl font-bold ${hl ? "text-white" : "text-[#0A2540]"}`}
            >
              —
            </p>
          )}
      ...
      {(tier.tagline ?? tier.description) && (
        <p
          className={`text-sm mb-5 leading-relaxed ${hl ? "text-white/60" : "text-foreground/70"}`}
        >
          {tier.tagline ?? ""}
        </p>
      )}
          ...
          {features.map((f, i) => (
            <li
              ...
              className={`flex items-start gap-2 text-sm ${
                hl ? "text-white/80" : "text-foreground"
              }`}
            >
      ...
      <div className="mt-auto pt-4">
        {contactUs ? (
          <Link href="/contact">
            <Button
              variant="outline"
              className={`w-full ${
                hl ? "border-white/30 text-white hover:bg-white/10" : ""
              }`}
            >
          ...
          <Link href={`/checkout?product=${tier.slug ?? ""}&seats=${seats}`}>
            <Button
              className={`w-full ${hl ? "" : "bg-[#0A2540] hover:bg-[#0A2540]/90"}`}
            >
        ...
        {!contactUs && hasCheckout && (
          <p
            className={`text-xs text-center mt-2 ${hl ? "text-white/40" : "text-muted-foreground"}`}
          >
...
export default function Monitoring() {
  ...
  return (
    <Layout>
      <SEOMeta
        title="Continuous Microsoft 365 Monitoring | Shane McCaw Consulting"
        description="NASA-informed Microsoft 365 monitoring that tracks configuration drift, security gaps, licence waste, and governance risks — every day."
      />
      ...
      <section className="bg-[#0A2540] pt-[130px] pb-20 px-6 text-center">
        <div className="max-w-[860px] mx-auto">
          <div className="inline-flex items-center gap-2 bg-white/10 text-white text-xs font-bold uppercase tracking-wider px-3 py-1.5 rounded-full mb-6">
            ...
            Your Assessment Was a Snapshot. Monitoring Keeps It Current.
          </div>

          <h1 className="text-4xl md:text-5xl font-extrabold text-white mb-6 leading-tight">
            Continuous Microsoft 365 oversight — powered by a NASA-informed
            engine.
          </h1>

          <p className="text-white/70 text-lg max-w-2xl mx-auto leading-relaxed mb-8">
            The same architecture used to secure NASA’s Copilot rollout —
            applied to your tenant every day. Signals fire when something
            changes. You act only when you need to.
          </p>

          <div className="flex flex-wrap justify-center gap-6 text-sm text-white/50">
            <span className="flex items-center gap-1.5">
              <CheckCircle2 className="size-4 text-[#00B4D8]" /> No agents to
              deploy
            </span>
            <span className="flex items-center gap-1.5">
              <CheckCircle2 className="size-4 text-[#00B4D8]" /> Weekly signal
              reports
            </span>
            <span className="flex items-center gap-1.5">
              <CheckCircle2 className="size-4 text-[#00B4D8]" /> Escalate
              findings to Shane
            </span>
          </div>
        </div>
      ...
      <section className="bg-white py-16 px-6 border-b border-border">
        <div className="max-w-[960px] mx-auto">
          <h2 className="text-center text-2xl font-extrabold text-[#0A2540] mb-10">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 text-center">
            <div className="flex flex-col items-center">
              ...
              <h3 className="text-lg font-bold text-[#0A2540] mb-2">
                Connect once (admin consent)
              </h3>
              <p className="text-sm text-foreground/70 leading-relaxed">
                Grant read-only access via Microsoft 365 admin consent. No
                agents, no software, no maintenance.
              </p>
            </div>
            ...
            <div className="flex flex-col items-center">
              ...
              <h3 className="text-lg font-bold text-[#0A2540] mb-2">
                The engine watches continuously
              </h3>
              <p className="text-sm text-foreground/70 leading-relaxed">
                A NASA-informed signal library evaluates your tenant daily —
                configuration, security, identity, licence, and governance
                checks.
              </p>
            </div>
            ...
            <div className="flex flex-col items-center">
              ...
              <h3 className="text-lg font-bold text-[#0A2540] mb-2">
                Act only when signals fire
              </h3>
              <p className="text-sm text-foreground/70 leading-relaxed">
                Weekly digests summarize changes. Critical signals trigger
                immediate alerts — with fixed-price remediation options.
              </p>
            </div>
          </div>
        </div>
      ...
      <section className="bg-[#F7F9FC] py-10 px-6 border-b border-border">
        <div className="max-w-[640px] mx-auto text-center">
          ...
          <p className="text-sm text-muted-foreground mb-5">
            Pricing adjusts live as you change the seat count.
          </p>
        ...
        <div className="max-w-[1200px] mx-auto">
          ...
          {error && !loading && (
            <div className="flex flex-col items-center gap-3 py-16 text-center">
              ...
              <p className="text-muted-foreground">
                Could not load monitoring packs. Please refresh and try again.
              </p>
            </div>
          )}
          ...
          {!loading && !error && sorted.length === 0 && (
            <div className="text-center py-16 text-muted-foreground">
              No monitoring packs available yet — check back soon.
            </div>
          )}

          {!loading && !error && sorted.length > 0 && visible.length === 0 && (
            <div className="text-center py-16 text-muted-foreground">
              No packs available for {seats} seats —{" "}
              <a href="/contact" className="underline text-[#0078D4]">
                contact us
              </a>{" "}
              for a custom quote.
            </div>
        ...
        <div className="max-w-[960px] mx-auto">
          <div className="text-center mb-12">
            ...
            <p className="text-muted-foreground max-w-xl mx-auto">
              Your tenant changes constantly — new users, policy edits, app
              registrations, licence assignments. Each one is a potential
              signal.
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
            ...
            <div className="bg-[#F7F9FC] rounded-xl p-6 border border-border">
              ...
              <p className="text-sm text-foreground/70 leading-relaxed">
                MFA gaps, legacy auth enabled, overly permissive Conditional
                Access, admin role sprawl.
              </p>
            </div>
            ...
            <div className="bg-[#F7F9FC] rounded-xl p-6 border border-border">
              ...
              <p className="text-sm text-foreground/70 leading-relaxed">
                Policies that changed without a change request — SharePoint
                sharing settings, Teams guest access, DLP rules.
              </p>
            </div>
            ...
            <div className="bg-[#F7F9FC] rounded-xl p-6 border border-border">
              ...
              <p className="text-sm text-foreground/70 leading-relaxed">
                Assigned but unused licences, duplicate SKUs, unactivated
                Copilot seats.
              </p>
            </div>
            ...
            <div className="bg-[#F7F9FC] rounded-xl p-6 border border-border">
              ...
              <p className="text-sm text-foreground/70 leading-relaxed">
                Stale guest accounts, unmanaged service accounts, orphaned
                mailboxes.
              </p>
            </div>
            ...
            <div className="bg-[#F7F9FC] rounded-xl p-6 border border-border">
              ...
              <p className="text-sm text-foreground/70 leading-relaxed">
                Retention policy gaps, audit log disabled, data governance
                weaknesses.
              </p>
            </div>
            ...
            <div className="bg-[#F7F9FC] rounded-xl p-6 border border-border">
              ...
              <p className="text-sm text-foreground/70 leading-relaxed">
                Data access oversharing, missing sensitivity labels, governance
