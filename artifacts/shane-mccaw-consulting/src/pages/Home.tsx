...
export default function Home() {
  ...
  const [activeCatalogTab, setActiveCatalogTab] = useState<'assessments' | 'monitoring' | 'retainers'>('assessments');
  ...
  const engines = [
    {
      id: 'drift',
      name: 'Drift Engine',
      description: 'Continuous automated configuration detection. Identifies unauthorized administrative adjustments and baseline deviations instantly.',
    ...
    },
    {
      ...
      description: 'Finds security vulnerabilities including anonymous links, stale guest permissions, over-privileged OAuth apps, and missing MFA parameters.',
    ...
    },
    {
      id: 'health',
      name: 'Health Engine',
      description: 'Calculates real-time tenant operational risk scores and runs automated remediation runbooks to correct discovered anomalies.',
    ...
    },
    {
      id: 'sla',
      name: 'SLA Engine',
      description: 'Tracks support delivery SLAs, response timelines, milestone execution rates, and uptime guarantees.',
    ...
    },
    {
      ...
      description: 'Monitors ongoing engineer workstreams against the legal SOW parameters to ensure all client operations are correctly accounted for.',
    ...
    },
    {
      ...
      description: 'Analyzes tenant telemetry gaps to dynamically calculate target-focused advisory upgrades and custom monitoring expansions.',
      ...
      <main className="flex-grow pt-24 pb-16">
        ...
        <section className="border-y border-slate-800/80 bg-slate-900/40 py-8 px-4 sm:px-6 lg:px-8">
          <div className="max-w-7xl mx-auto flex flex-col md:flex-row items-center justify-between gap-6 text-center md:text-left">
            ...
            <div className="text-sm text-slate-300 max-w-md md:text-right">
              Platform calculates risk, configuration drift, and regulatory compliance dynamically using telemetry harvested from Microsoft Graph.
            </div>
          </div>
        </section>
        ...
        <section className="py-20 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">
          <div className="text-center max-w-3xl mx-auto mb-16">
            <h2 className="text-3xl sm:text-4xl font-bold text-white mb-4">Continuous Automated Telemetry Analysis</h2>
            <p className="text-slate-400">Six specialized operational engines inspect tenant configuration parameters to enforce compliance standards at federal scale.</p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {engines.map((eng) => {
              const Icon = eng.icon;
              return (
                <div key={eng.id} className="p-6 rounded-2xl bg-slate-900/60 border border-slate-850 hover:border-blue-500/20 transition-all flex flex-col">
                  <div className="w-10 h-10 rounded-xl bg-blue-500/10 border border-blue-500/20 flex items-center justify-center mb-4">
                    <Icon className="w-5 h-5 text-blue-400" />
                  </div>
                  <span className="text-[10px] uppercase font-bold tracking-wider text-blue-400 mb-1">{eng.badge}</span>
                  <h3 className="text-lg font-bold text-white mb-2">{eng.name}</h3>
                  <p className="text-sm text-slate-400 leading-relaxed">{eng.description}</p>
                </div>
              );
            })}
          </div>
        ...
        <section className="py-16 bg-slate-950/60 border-t border-slate-800/80 px-4 sm:px-6 lg:px-8">
          <div className="max-w-7xl mx-auto">
            <div className="flex flex-col md:flex-row md:items-end justify-between mb-12 gap-6">
              <div>
                <h2 className="text-3xl font-bold text-white mb-3">M365 Catalog & Packages</h2>
                <p className="text-slate-400 max-w-xl">Browse dynamically updated, fixed-price assessments and monitoring deliverables retrieved directly from the catalog database.</p>
              </div>
            ...
            <div className="flex justify-center">
              ...
              {activeCatalogTab === 'monitoring' && renderProductGrid(monitoringPackages)}
        ...
        <section className="py-20 px-4 sm:px-6 lg:px-8 max-w-5xl mx-auto text-center">
          <div className="p-8 sm:p-12 rounded-3xl bg-gradient-to-b from-blue-950/20 to-slate-900 border border-blue-500/10 shadow-2xl relative overflow-hidden">
            ...
            <h2 className="text-3xl sm:text-4xl font-extrabold text-white mb-4 relative z-10">Ready to Secure & Govern Your M365 Tenant?</h2>
            <p className="text-slate-300 max-w-xl mx-auto mb-8 relative z-10 text-sm sm:text-base">
              Establish NASA-grade security framework compliance. Deploy our automated telemetry monitoring or schedule a targeted assessment package to verify baseline health.
            </p>
            <div className="flex flex-col sm:flex-row justify-center gap-4 relative z-10 max-w-xs sm:max-w-none mx-auto">
              <Link href="/assessments" className="px-6 py-3.5 rounded-xl font-bold bg-blue-600 hover:bg-blue-500 text-white transition-colors">
                Explore M365 Catalog
              </Link>
              <Link href="/contact" className="px-6 py-3.5 rounded-xl font-semibold bg-slate-800 hover:bg-slate-700 text-slate-200 border border-slate-705 transition-colors">
                Contact Shane McCaw
              </Link>
            </div>
          </div>
