...
export function CalendarBooking() {
  ...
  if (step === "success" && selectedSlot) {
    return (
      <div className="w-full h-full flex items-center justify-center min-h-[600px]">
        <div className="bg-white rounded-2xl border border-[#0078D4]/20 p-12 text-center max-w-md w-full shadow-sm">
          ...
          <h2 className="text-2xl font-bold text-[#0A2540] mb-3">You're booked!</h2>
          <p className="text-[#0A2540] font-medium mb-1">{formatSlotDate(selectedSlot.startIso)}</p>
          <p className="text-muted-foreground text-sm mb-6">30 minutes · Microsoft Teams / Phone call</p>
  ...
  return (
    <div className="w-full bg-white rounded-2xl border border-border overflow-hidden shadow-sm">
      ...
      {/* ── Step indicator ── */}
      <div className="border-b border-border bg-[#F7F9FC] px-6 py-4 flex items-center gap-6">
            ...
            <div key={s} className="flex items-center gap-2">
              <span
                className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold flex-shrink-0 transition-colors
                  ${done ? "bg-[#0078D4] text-white" : active ? "bg-[#0A2540] text-white" : "bg-slate-200 text-slate-500"}`}
              >
                {done ? "✓" : idx + 1}
              </span>
              <span className={`text-sm font-medium hidden sm:inline ${active ? "text-[#0A2540]" : "text-muted-foreground"}`}>
                {labels[idx]}
              </span>
            </div>
          );
      ...
      <div className="p-6">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
              {businessDays.map((day) => (
                <button
                  ...
                  className="rounded-lg border border-border bg-white hover:border-[#0078D4] hover:bg-[#0078D4]/5 transition-colors p-3 text-left"
                >
                  <span className="block text-sm font-semibold text-[#0A2540]">{day.label}</span>
                </button>
              ))}
            </div>
          </div>
        ...
        {step === "time" && selectedDate && (
          <div>
            <button
              ...
              className="flex items-center gap-1 text-sm text-muted-foreground hover:text-[#0A2540] mb-4 transition-colors"
            >
              <ChevronLeft className="w-4 h-4" />
              Back to dates
            </button>
            <p className="text-sm font-semibold text-[#0A2540] mb-1">{selectedDate.label}</p>
            <p className="text-xs text-muted-foreground mb-5">All times shown in Eastern Time (ET)</p>
              ...
              <div className="grid grid-cols-3 sm:grid-cols-4 gap-3">
                {slots.map((slot) => (
                  <button
                    ...
                    className="rounded-lg border border-border bg-white hover:border-[#0078D4] hover:bg-[#0078D4]/5 transition-colors p-3 text-sm font-medium text-[#0A2540]"
                  >
                    {slot.label}
                  </button>
                ))}
        ...
        {step === "form" && selectedSlot && selectedDate && (
          <div>
            ...
            <div className="rounded-xl bg-[#0078D4]/5 border border-[#0078D4]/20 p-4 flex items-center gap-3 mb-6">
              <Calendar className="w-5 h-5 text-[#0078D4] flex-shrink-0" />
              <div>
                <p className="text-sm font-semibold text-[#0A2540]">{selectedDate.label} · {selectedSlot.label} ET</p>
                <p className="text-xs text-muted-foreground">30 minutes · Microsoft Teams / Phone call</p>
              </div>
            </div>
            ...
            <form onSubmit={(e) => { void handleSubmit(onSubmit)(e); }} className="space-y-4">
              <div>
                ...
                <input
                  ...
                  className="w-full rounded-lg border border-border bg-white px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#0078D4]/30 focus:border-[#0078D4] transition"
                />
              ...
              <div>
                <label className="block text-sm font-medium text-[#0A2540] mb-1">
                  Topic / agenda <span className="text-rose-500">*</span>
                </label>
                <textarea
                  {...register("topic")}
                  rows={4}
                  ...
                  className="w-full rounded-lg border border-border bg-white px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#0078D4]/30 focus:border-[#0078D4] transition resize-none"
                />
                {errors.topic && <p className="text-xs text-rose-500 mt-1">{errors.topic.message}</p>}
              ...
              <button
                ...
                className="w-full flex items-center justify-center gap-2 bg-[#0078D4] hover:bg-[#0078D4]/90 disabled:opacity-60 text-white font-semibold text-sm py-3 rounded-lg transition-colors"
              >
                {isSubmitting ? (
                  <>
