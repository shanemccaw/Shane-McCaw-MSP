 ...
 *
 * MSP-authenticated (any rung — `ladder.free`):
 ...
 *
 * Restored by #4048 (reversal of #3412). Re-gated against the RBAC ladder
 * (`requireCapability`/`ladder.*`) that #2460 established as the live pattern —
 * `requireRole` this file used pre-#3412 no longer exists.
 */
...
router.get("/platform/agreement/acceptance-status", requireCapability("ladder.free"), async (req: Request, res: Response) => {
  const user = req.user!;
...
router.post("/platform/agreement/accept", requireCapability("ladder.free"), async (req: Request, res: Response) => {
  const user = req.user!;
