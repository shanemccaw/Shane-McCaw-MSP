...
function parseSowPricing(html: string): { lines: SowPricingLine[]; totalPrice: number } {
  ...
  for (const tableHtml of tableMatches) {
    // Locate header row (inside <thead> or first <tr>)
    ...
    // Only process tables that look like pricing tables
    if (!headerText.includes("price") && !headerText.includes("fixed") && !headerText.includes("cost")) continue;

    // Determine column indices from header cells
    ...
    for (const row of rows) {
      ...
      // Parse the dollar amount — strip everything except digits and decimal point
