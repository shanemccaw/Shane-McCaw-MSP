...
// #3424 (Decommission legacy Clients/CRM admin-panel suite): the client
// CRUD, detail, app-registration, m365-profile, MFA, and document-hub routes
// that only ever backed the deleted Clients.tsx / ClientDetail.tsx /
// ClientM365HealthTab.tsx admin pages (and the orphaned M365ProfileWizard.tsx)
// have been removed along with them. What remains below is read-only surface
// still consumed by other live admin-panel pages (Reports, Invoices,
// Projects/ProjectDetail, EmailActivity, the real Kanban board's client
// picker via KanbanCardModal, TenantSignals, the workflow builders) and the
// mcp-server tools (whoami / query_customers / get_customer_findings).
