// artifacts/api-server/src/routes/admin-msp-directory.ts
//
// MSP Directory admin surface — Tenant/Customer/MSP/User/Group management
// routes (initiative: active-directory, docs/build-plans/active-directory.md).
// Split out of admin-active-directory.ts (Git #4495): the real OU/policy-engine
// routes (`/admin/active-directory/ou*`, `/admin/active-directory/ou-assignments*`)
// stayed behind in admin-active-directory-ou.ts, unchanged path/behavior. Every
// route here that previously lived under `/admin/active-directory/*` now lives
// under `/admin/msp-directory/*` — relocation + rename only, zero behavior
// change.
//
...
import {
  ...
  // but NO userId/clientUserId column of their own, so the shared per-user
  // cascade (`../lib/user-hard-delete.ts`) never reaches them. Audited fresh
  // against lib/db/src/schema
...
import { hardDeleteUserWithinTx } from "../lib/user-hard-delete.ts";
...
const log = logger.child({ channel: "admin.msp-directory" });
...
// intentional duplicate", and `DirUserCanvas` carried a third copy. The two
...
router.get("/admin/msp-directory/roles", requireAdmin, (_req: Request, res: Response) => {
  res.json({
...
// ─── GET /admin/msp-directory/search?q= ───────────────────────────────────
...
router.get("/admin/msp-directory/search", requireAdmin, async (req: Request, res: Response) => {
...
// ─── GET /admin/msp-directory/msp/:id ─────────────────────────────────────
...
router.get("/admin/msp-directory/msp/:id", requireAdmin, async (req: Request, res: Response) => {
...
// ─── GET /admin/msp-directory/group/:role?q= ──────────────────────────────
...
router.get("/admin/msp-directory/group/:role", requireAdmin, async (req: Request, res: Response) => {
...
// ─── GET /admin/msp-directory/customer/:id ────────────────────────────────
...
router.get("/admin/msp-directory/customer/:id", requireAdmin, async (req: Request, res: Response) => {
...
// ─── PATCH /admin/msp-directory/customer/:id ───────────────────────────────
...
router.patch("/admin/msp-directory/customer/:id", requireAdmin, async (req: Request, res: Response) => {
  const customerId = Number(req.params.id);
  if (!Number.isInteger(customerId)) {
    res.status(400).json({ error: "Invalid customer id" });
    return;
...
// ─── POST /admin/msp-directory/customer/:id/assign-service ────────────────
...
router.post("/admin/msp-directory/customer/:id/assign-service", requireAdmin, async (req: Request, res: Response) => {
  const customerId = Number(req.params.id);
  if (!Number.isInteger(customerId)) {
    res.status(400).json({ error: "Invalid customer id" });
    return;
...
// ─── GET /admin/msp-directory/customer/:id/diagnostics/runs ───────────────
...
router.get("/admin/msp-directory/customer/:id/diagnostics/runs", requireAdmin, async (req: Request, res: Response) => {
  const customerId = Number(req.params.id);
  if (!Number.isInteger(customerId)) {
    res.status(400).json({ error: "Invalid customer id" });
    return;
...
// ─── GET /admin/msp-directory/user/:id ────────────────────────────────────
...
router.get("/admin/msp-directory/user/:id", requireAdmin, async (req: Request, res: Response) => {
...
router.patch("/admin/msp-directory/user/:id/role", requireAdmin, async (req: Request, res: Response) => {
  const userId = Number(req.params.id);
  if (!Number.isInteger(userId)) {
    res.status(400).json({ error: "Invalid user id" });
    return;
...
router.patch("/admin/msp-directory/user/:id/assignment", requireAdmin, async (req: Request, res: Response) => {
  const userId = Number(req.params.id);
  if (!Number.isInteger(userId)) {
    res.status(400).json({ error: "Invalid user id" });
    return;
...
// GET /admin/msp-directory/user/:id/entitlements
...
router.get("/admin/msp-directory/user/:id/entitlements", requireAdmin, async (req: Request, res: Response) => {
  const userId = Number(req.params.id);
  if (!Number.isInteger(userId)) {
    res.status(400).json({ error: "Invalid user id" });
    return;
...
// PATCH /admin/msp-directory/user/:id/entitlements
...
router.patch("/admin/msp-directory/user/:id/entitlements", requireAdmin, async (req: Request, res: Response) => {
  const userId = Number(req.params.id);
  if (!Number.isInteger(userId)) {
    res.status(400).json({ error: "Invalid user id" });
    return;
...
router.post("/admin/msp-directory/user/:id/force-password-reset", requireAdmin, async (req: Request, res: Response) => {
  const userId = Number(req.params.id);
  if (!Number.isInteger(userId)) {
    res.status(400).json({ error: "Invalid user id" });
    return;
...
router.post("/admin/msp-directory/user/:id/mfa-reset", requireAdmin, async (req: Request, res: Response) => {
  const userId = Number(req.params.id);
  if (!Number.isInteger(userId)) {
    res.status(400).json({ error: "Invalid user id" });
    return;
...
router.post("/admin/msp-directory/user/:id/impersonate", requireAdmin, async (req: Request, res: Response) => {
  const userId = Number(req.params.id);
  if (!Number.isInteger(userId)) {
    res.status(400).json({ error: "Invalid user id" });
    return;
...
// ─── DELETE /admin/msp-directory/user/:id — Phase 9 (Issue #69) ───────────
...
//
// The per-user cascade this route runs — the full table footprint, and why each row is
// deleted / cascaded / attribution-blanked — now lives in ONE place, with its own
// documentation: `../lib/user-hard-delete.ts`. It moved there for Git #2984, so the
// scheduled 7-year post-termination purge (which has no human actor) can run the exact
// same cascade this route does rather than a second, drifting copy of it.
//
// The FULL pre-delete state (row counts for every table it touches, cascades or blanks)
// is logged to the `audit` channel by that function BEFORE the transaction commits, and
// this route is additionally gated server-side to non-production environments — fail
// closed. That gate is the ROUTE's, not the cascade's: the retention purge is a
// production behaviour by design.
...
router.delete("/admin/msp-directory/user/:id", requireAdmin, async (req: Request, res: Response) => {
  ...
  try {
    const result = await db.transaction((tx) => hardDeleteUserWithinTx(tx, userId, { kind: "user", id: actor.id, email: actor.email }));
...
// ─── DELETE /admin/msp-directory/customer/:id — customer hard delete ─────
...
router.delete("/admin/msp-directory/customer/:id", requireAdmin, async (req: Request, res: Response) => {
  // FIRST executed line: the hard non-production gate, before anything else.
  const envRefusal = assertNonProductionEnvironment();
  if (envRefusal) {
    auditLog.warn(
  ...
  try {
    phaseAResult = await db.transaction(async (tx) => {
      ...
      for (const u of userRows) {
        const userResult = await hardDeleteUserWithinTx(tx, u.id, { kind: "user", id: actor.id, email: actor.email });
