...
router.post(
  ...
  async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      // #1171 — a Project Work (service_class='project') offer acceptance
      // produces the real engagement: a projects row plus a SOW generated by the
      // proven document-engine-sow.ts pipeline (AI-priced, findings-bound), NOT
      // a flat catalog price and NOT a second SOW engine. Fire-and-forget: the
      // offer is already `accepted`, so a fulfillment failure must never turn
      // this response into an error, and the AI generation must not block it.
      // Non-project offers no-op inside the helper.
      void fulfillAcceptedProjectOffer({ offerId: id, acceptedByUserId: actorId }).catch((err) => {
        log.error({ err, offerId: id }, "portal-offers: project SOW fulfillment kickoff failed");
      });
