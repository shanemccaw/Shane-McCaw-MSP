...
const FLAT_NODES = [
  ...
  nodeDef("create_kanban_task", "Create Kanban Task", "Project / Kanban", "Creates a new task card on a project kanban board.", [{ key: "boardId", type: "string", required: true, description: "Numeric project board ID (or {{token}})." }, { key: "title", type: "string", required: true, description: "Task title." }, { key: "column", type: "string", description: "Column slug." }, { key: "priority", type: "string", description: "Priority." }, { key: "body", type: "string", description: "Task description." }], ["taskId", "boardId", "columnId", "title"], ["default"]),
  ...
  nodeDef("evaluate_engagement_offers", "Evaluate Engagement Offers", "Engagement Offer Engine", "Evaluates all active Engagement Offer Engine rules against every lead with activity in the last 24 hours, firing configured workflow events for anything that crosses its behavioral thresholds (subject to per-rule cooldown). Takes no input — operates globally across all leads.", [], ["leadsChecked", "totalFired"], ["default"]),
  nodeDef("dispatch_engagement_followups", "Dispatch Engagement Followups", "Engagement Offer Engine", "Finds engagement_offer_firings rows with no follow-up dispatched yet (fired in the last 24 hours) and spawns a Delayed Follow-Up workflow run for each, marking it dispatched. Takes no input — operates globally across all pending firings.", [], ["checked", "dispatched"], ["default"]),
  nodeDef("cancel_conflicting_engagement_followup", "Cancel Conflicting Engagement Followup", "Engagement Offer Engine", "Resolves the purchaser (clientId) to a lead via email, and if a pending Delayed Follow-Up run exists for that lead whose rule eligibleServiceIds overlaps the purchased serviceIds, cancels it — never send a bundle-discount offer for a service the lead already bought at full price.", [{ key: "clientId", type: "string", description: "Purchaser user ID from the purchase.completed payload. Supports templates." }, { key: "serviceIds", type: "string", description: "Comma-separated purchased service IDs from the purchase.completed payload. Supports templates." }], ["checked", "matched", "cancelledCount", "leadId", "cancelledRunIds"], ["default"]),
  // ── Scope Creep Engine ────────────────────────────────────────────────────
  ...
  nodeDef("create_phased_invoices", "Create Phased Invoices", "Commerce / Stripe", "Creates Stripe draft invoices for a paid Pay-by-Phase checkout session's remaining phases (2..N), crediting the deposit against the final phase.", [{ key: "checkoutSessionId", type: "string", required: true, description: "checkout_sessions row id (uuid) — must be a paid phased plan with a signed assessment_sow_agreements row." }], ["invoiceIds", "phaseCount", "totalScheduled", "datesFlagged", "skippedNonPositivePhases"], ["default"]),
  ...
  nodeDef("policy_evaluate_due", "Policy Engine — Evaluate Due Standing Policies", "MSP / System", "Policy Engine continuous-evaluation reconciliation pass (#1549). Scoped to one tenant when the triggering run carries a customerId (the EVENT trigger — a standing policy was just switched on), or swept across every opted-in tenant when it does not (the DIVERGENCE trigger — the hourly schedule). Gated on the policy's own is_active AND the tenant's policy_engine_opt_in; reuses #1553's real compliance evaluator and records one policy_evaluation_runs row per policy considered.", [], ["triggerKind", "triggerEventType", "scopedCustomerId", "policiesConsidered", "compliant", "divergent", "notEvaluable", "skippedNotOptedIn", "errors"], ["default"]),
