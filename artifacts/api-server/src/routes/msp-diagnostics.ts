...
import { requireCapability, requireAuth, assertCustomerAccess, isCustomerBlockedByStaffScope, type AuthUser } from "../middlewares/requireAuth.ts";
import { logger } from "../lib/logger.ts";
const log = logger.child({ channel: "tenant.portal" });
import { runDiagnostics } from "../lib/diagnostics-runner.ts";
import { runItemDetailCollection } from "../lib/item-detail-collector.ts";
import { registerDiagnosticsRunSSEClient } from "../lib/sse-channels.ts";
import { calculateArchitectureHealthScore } from "../lib/health-engine.ts";
import { computeDisplayHealth } from "../lib/health-display.ts";
