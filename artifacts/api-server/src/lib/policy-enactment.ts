 ...
 * run into a real, auto-approved `standard` change request: the same
 * `changeClass: "standard"` / `approverRole: "catalog_inherited"` machinery
 * `routes/portal-change-catalog.ts`'s customer-facing "execute" already uses
 * for self-service, applied here to the MSP-side, policy-triggered path.
 *
