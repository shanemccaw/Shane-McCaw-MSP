/**
 * Duplicated verbatim from `artifacts/admin-panel/src/lib/scriptPoller.ts`
 * (Git #4246), not moved — that copy still backs admin-panel's own
 * `RunLibraryScriptDialog.tsx` (used by the unrelated `ScriptGeneratorPage.tsx`)
 * and `KanbanCardModal.tsx`/`TypedCardContent.tsx` there, so it can't be
 * deleted. Kept in sync by hand until a `lib/*` package exists for it.
 */
