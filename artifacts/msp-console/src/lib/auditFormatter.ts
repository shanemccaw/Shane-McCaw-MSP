/**
 * Duplicated verbatim from `artifacts/admin-panel/src/lib/auditFormatter.ts`
 * (Git #4246), not moved — that copy still backs admin-panel's own, unrelated
 * `pages/ActivityLog.tsx`, so it can't be deleted there. Real behavior, real
 * shared shape, kept in sync by hand until a `lib/*` package exists for it.
 */
