...
test("IA has 7 tenant groups and 13 ops pages", () => {
  assert.equal(CHILD_GROUPS.length, 7);
  assert.equal(MSP_PAGES.length, 13);
...
test("collapsed tree shows two roots + operations pages, tenants closed", () => {
  ...
  // 12 operations children (settings lives on the Consulting root)
...
test("command palette lists root, every ops page, and tenant × page", () => {
  ...
  // 1 root + 13 ops + 3 tenants * 24 pages
  assert.equal(cmds.length, 1 + 13 + customers.length * 24);
