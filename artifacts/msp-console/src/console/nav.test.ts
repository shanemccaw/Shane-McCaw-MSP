...
test("IA has 7 tenant groups and 9 ops pages", () => {
  assert.equal(CHILD_GROUPS.length, 7);
  assert.equal(MSP_PAGES.length, 9);
  // 2 leaf groups (overview, audit) + 3 + 7 + 4 + 4 + 3 group children = 23
  assert.equal(CHILD_PAGES.length, 23);
});
...
test("collapsed tree shows two roots + operations pages, tenants closed", () => {
  ...
  // 8 operations children (settings lives on the Consulting root)
...
test("command palette lists root, every ops page, and tenant × page", () => {
  ...
  // 1 root + 9 ops + 3 tenants * 23 pages
  assert.equal(cmds.length, 1 + 9 + customers.length * 23);
