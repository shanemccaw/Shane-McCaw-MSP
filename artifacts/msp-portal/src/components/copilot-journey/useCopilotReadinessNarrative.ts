/**
 * useCopilotReadinessNarrative.ts — the Copilot Readiness report's three
 * AI-written sections (#409).
 *
 * Separate from `useCopilotJourney` on purpose. That hook backs all four
 * journey screens and polls; this one makes up to three real Anthropic calls
 * server-side, so it fires ONCE, only when the Copilot Readiness report is
 * actually open, and never polls. The report's pure-data sections do not wait
 * on it — they render the moment the pillar payload lands, and each prose
 * section shows its own pending state until this settles.
 *
 ...
 * the first of those may show a spinner. A failed fetch settles too — the
 * report then renders its honest unavailable state rather than spinning
...
import type { WireNarrativePayload } from "./copilotReadinessReport.ts";

const NARRATIVE_URL = "/api/portal/assessment/copilot-readiness-narrative";
const JOURNEY_CHANNEL = "engine.dashboard";

export interface CopilotReadinessNarrativeState {
...
export function useCopilotReadinessNarrative(options: {
  /** Nothing is fetched until the report is genuinely on screen. */
  readonly enabled: boolean;
}): CopilotReadinessNarrativeState {
  const { fetchWithAuth, accessToken } = useAuth();
  const [narrative, setNarrative] = useState<WireNarrativePayload | null>(null);
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (!res.ok) throw new Error(`copilot readiness narrative ${res.status}`);
      setNarrative((await res.json()) as WireNarrativePayload);
    } catch (e) {
      ...
      reportClientEvent(tokenRef.current, "CopilotReadinessNarrativeFetchFailed", message, JOURNEY_CHANNEL);
    } finally {
