...
import { LiveFinding } from './useM365HealthLive';

/**
 * Remediation surface — the honest replacement for the mock "Automated
 * Remediation Engine" theater (fake 3-step execution + fake score bump).
 *
 * What this really does: for a finding with a server-linked sales offer
 * (mission-control overview's real finding↔offer linkage), it surfaces that
 * real offer — title, rationale, real engine-adjusted price — and routes to
 * /customer-offers, where the offer is genuinely actionable through the
 * existing accept/checkout flow. For a finding without an offer, it shows the
 * finding's own real recommended action/effort from the diagnostics
 * recommendation.
 *
 * Live one-click execution is deliberately NOT wired: automated remediation
 * runs real Graph writes and is blocked pending the tenant's Azure app
 * registration (server-side the /portal/mission-control/remediate endpoint is
 * hard-gated to testbed tenants for exactly this reason). The modal says so
 * plainly instead of pretending to execute.
 */
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm">
      <div className="bg-popover border border-border rounded-2xl w-full max-w-lg overflow-hidden shadow-2xl">
        ...
        <div className="p-6 space-y-5">
          ...
          {/* Real recommended action from the diagnostics recommendation */}
          ...
          {/* Honest execution-blocked state — no fake automation */}
