...
import { X, AlertTriangle, ShieldCheck, Wrench, Sparkles } from 'lucide-react';
...
function mockRemediation(issue: UseCaseIssue): string[] {
  return [
    'Placeholder remediation step one -- real guidance will be specific to this exact finding.',
    'Placeholder remediation step two, sequenced by effort and impact.',
    'Placeholder step three, including the PowerShell/Graph action that resolves it once available.',
  ];
}
        ...
        <div className="space-y-2">
          ...
          <div className="space-y-2">
            {mockRemediation(issue).map((step, idx) => (
              <div
                key={idx}
                className="flex items-start space-x-2.5 bg-muted/50 border border-border p-3 rounded-md text-xs text-foreground"
              >
