...
interface AssessmentStatus {
  ...
  // gateRequired is the real prod/dev signal (Git #439, REPLIT_DOMAINS —
  // same formula as stripe.ts's getStripeKey): dev-server test logins are
  // never gated, production logins/account-creation always are.
  mfa: { enrolled: boolean; gateRequired: boolean };
...
export function AssessmentWizard() {
  const { user, accessToken, fetchWithAuth, extendSession } = useAuth();
  ...
  // ── Mandatory MFA gate ─────────────────────────────────────────────────────
  // Block the entire flow until the customer enrolls a portal-login second
  // factor. gateRequired is real (production only, Git #439) — the backend
  // also enforces this at every other route while the session is
  // mfaSetupPending, so this is UX, not the actual security boundary.
  if (status?.mfa.gateRequired && loaded && status && !status.mfa.enrolled && !mfaJustEnrolled) {
    return (
      <AssessmentMfaEnrollment
        onEnrolled={() => {
          setMfaJustEnrolled(true);
          // The access token issued at account creation is mfaSetupPending —
          // refresh it now so the rest of the wizard's real API calls (SOW
          // selection, payment, scan triggers) aren't refused by requireAuth's
          // enrollment-only allowlist for the remainder of its 15-minute life.
          void extendSession();
          void loadStatus();
        }}
      />
