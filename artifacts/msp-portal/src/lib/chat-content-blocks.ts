/**
 * ShaneBot structured content blocks — client mirror (#361).
 *
 * The canonical shape lives server-side (`@workspace/db`'s ChatContentBlock, with
 * helpers in api-server's `lib/chat-content-blocks.ts`). msp-portal is an
 * independent Vite app with no workspace-package dependency, so the shape is
 * restated here rather than imported. Keep the two in step — the type union and
 * the legacy-tolerant normalizer are the parts that must not drift.
 */
...
/**
 * Normalize any content to blocks. Chats saved to localStorage before #361 hold
 * bare strings, so every render/read path must go through this rather than
 * assuming an array — that tolerance is what makes a backfill unnecessary.
 */
...
/** Flatten content to plain text — what the bubble renders and what the API is sent. */
