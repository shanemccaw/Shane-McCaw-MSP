 ...
 * surface for building a dashboard_templates canvas layout. Renders the
 * shared <DashboardCanvas> (from @workspace/dashboard-canvas, Step 4a) in
 * edit mode with the mock data fetcher for live preview — the real
 * POST /api/dashboard/resolve endpoint is deliberately not wired here, that's
 * a separate follow-up once this designer + the customer/MSP viewer exist.
 *
 * Flow: pick an MSP -> pick a templateType -> (for assessment/project/
 * monitoring_package) pick a targetKey -> load the existing template for that
 * combination if one exists, else start blank -> add widgets from the metric
 * palette -> drag/resize on the canvas -> Save Template.
 */
...
// DASHBOARD_METRICS keys are dot-namespaced (e.g. "identity.mfaRegisteredCount")
// — the part before the first "." is the category.
...
function nextWidgetPosition(widgets: WidgetInstance[], w: number, h: number): { x: number; y: number } {
...
function WidgetPalette({ onPick }: { onPick: (metric: MetricDef) => void }) {
...
export default function DashboardDesignerPage() {
  ...
  const [monitoringPackages, setMonitoringPackages] = useState<MonitoringPackageOption[]>([]);

  useEffect(() => {
    if (templateType !== "monitoring_package") return;
    (async () => {
      const res = await fetchWithAuth("/api/admin/monitoring-packages");
      if (!res.ok) return;
      const data = (await res.json()) as { packages: MonitoringPackageOption[] };
      setMonitoringPackages(data.packages ?? []);
    })();
  }, [templateType, fetchWithAuth]);

  const assessmentServices = useMemo(
    () => (services ?? []).filter((s) => detectProductType(s.serviceClass, s.deliveryType, s.billingType, s.fulfillmentType) === "assessment"),
    [services],
  );
  const projectServices = useMemo(
    () => (services ?? []).filter((s) => detectProductType(s.serviceClass, s.deliveryType, s.billingType, s.fulfillmentType) === "project"),
    [services],
  );

  const needsTargetKey = TARGET_KEY_TEMPLATE_TYPES.has(templateType);

  // Reset targetKey whenever templateType changes so a stale key from a
  // different type/list can't leak through.
  useEffect(() => {
    setTargetKey(null);
  }, [templateType]);
  ...
  const loadTemplate = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetchWithAuth(`/api/admin/dashboard-templates/lookup?${params}`);
      if (!res.ok) {
        toast({ title: "Error", description: "Failed to look up existing template", variant: "destructive" });
        return;
      }
  ...
  }, [canLoad, mspId, templateType, targetKey, fetchWithAuth, toast]);
  ...
  function handlePickMetric(metric: MetricDef) {
    ...
    // More than one legal renderer — the "same data, different chart type"
    // capability. Don't silently default; ask which one.
  ...
  async function handleSave() {
    if (mspId == null) {
      toast({ title: "Pick an MSP first", variant: "destructive" });
      return;
    }
    if (needsTargetKey && !targetKey) {
      toast({ title: `A ${TEMPLATE_TYPES.find((t) => t.value === templateType)?.label} target is required`, variant: "destructive" });
      return;
    }
    setSaving(true);
    try {
      const res = await fetchWithAuth("/api/admin/dashboard-templates", {
        method: "POST",
      ...
      setExistingTemplateId(data.template.id);
      toast({ title: existingTemplateId ? "Template updated" : "Template created" });
    } finally {
      setSaving(false);
    }
  }

  const targetKeyOptions: { value: string; label: string }[] =
    templateType === "assessment"
      ? assessmentServices.map((s: ServiceRow) => ({ value: s.slug ?? String(s.id), label: s.name }))
      : templateType === "project"
        ? projectServices.map((s: ServiceRow) => ({ value: s.slug ?? String(s.id), label: s.name }))
        : templateType === "monitoring_package"
          ? monitoringPackages.filter((p) => p.status === "active").map((p) => ({ value: p.key, label: p.label }))
          : [];
  ...
  return (
    <div className="p-6 h-[calc(100vh-4rem)] flex flex-col gap-4 overflow-hidden">
      ...
      <div className="flex-1 flex gap-4 overflow-hidden min-h-0">
        <WidgetPalette onPick={handlePickMetric} />
...
// <DashboardCanvas> itself is generic/viewer-agnostic (Step 4a); the designer
// needs a couple of edit-only affordances (remove, count/% toggle) layered on
// top without reaching into the shared component.
...
function DashboardCanvasEditor({
  ...
  return (
    <div className="relative">
      {/* Per-widget edit controls float above the grid, keyed by widget id, so
          <DashboardCanvas>'s own DOM stays untouched by designer-only chrome. */}
