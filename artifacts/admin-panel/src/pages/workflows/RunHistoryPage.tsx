...
function StatusChip({ status }: { status: string }) {
  return (
    <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-semibold border ${STATUS_STYLES[status] ?? "bg-accent text-muted-foreground border-border"}`}>
...
function RunRow({
  ...
  return (
    <div className="border-b border-border/50">
      <div
        className={`${GRID} px-4 py-3 hover:bg-accent transition-colors cursor-pointer items-center`}
        onClick={onClick}
      >
        <span className="text-xs text-muted-foreground/60 font-mono">{run.id}</span>

        <div className="min-w-0">
          <p className="text-sm font-medium text-foreground truncate">{run.definitionName ?? "—"}</p>
          {run.versionLabel && (
            <p className="text-[10px] text-muted-foreground/60 truncate">{run.versionLabel}</p>
          )}
        </div>
          ...
          {run.triggerType === "event" && run.triggerRef && run.triggerRef !== "draft_test" ? (
            <div className="flex flex-col gap-0.5 min-w-0">
              <span className="flex items-center gap-1">
                ...
                <span className="text-xs text-foreground font-mono truncate">{run.triggerRef}</span>
              </span>
              {(() => {
            ...
            <span className="text-sm">
              ...
              <span className="text-xs text-muted-foreground capitalize">{run.triggerType}</span>
            </span>
          )}
        ...
        <span className="text-xs text-muted-foreground font-mono">{fmtDuration(run.durationMs)}</span>
        ...
        <div className="flex items-center gap-1.5 justify-end" onClick={e => e.stopPropagation()}>
          {(run.status === "failed" || run.status === "cancelled" || run.status === "completed") && onRerun && (
            <button
              ...
              className="p-1 rounded hover:bg-primary/20 text-muted-foreground/60 hover:text-primary disabled:opacity-40 disabled:cursor-not-allowed transition-colors"
            >
              {isRerunning ? (
    ...
    <div className="h-full overflow-y-auto p-6">
      <div className="max-w-5xl mx-auto space-y-6">
        ...
        <div className="space-y-2">
          <div className="flex items-center gap-3 flex-wrap">
            <select
              ...
              className="bg-card border border-border rounded-lg px-3 py-1.5 text-sm text-foreground outline-none focus:border-primary/60"
            >
            ...
            <div className="flex items-center gap-1.5">
              ...
              <input
                ...
                className="bg-card border border-border rounded-lg px-2 py-1.5 text-sm text-foreground outline-none focus:border-primary/60 [color-scheme:dark]"
              />
            </div>
            <div className="flex items-center gap-1.5">
          ...
          <div className="flex items-center gap-3 flex-wrap">
            <div className="flex items-center gap-1.5">
              <span className="text-xs text-muted-foreground/60">Trigger</span>
              <select
        ...
        <div className="bg-card border border-border rounded-xl overflow-hidden">
          <div className={`${GRID} px-4 py-2.5 border-b border-border text-xs font-semibold text-muted-foreground/60 uppercase tracking-wider`}>
            <span>#</span>
            <span>Workflow</span>
            <span>Trigger</span>
            <span>Status</span>
          ...
          {isLoading ? (
            <div className="space-y-px">
              {[...Array(8)].map((_, i) => (
                <div key={i} className="h-12 bg-background/40 animate-pulse" />
              ))}
            </div>
          ) : allRuns.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-16 text-center">
              <p className="text-muted-foreground font-medium">No runs yet</p>
              <p className="text-sm text-muted-foreground/60 mt-1">Trigger a workflow manually or via a schedule.</p>
            </div>
          ) : (
            <>
              {/* User workflow runs */}
              ...
              {systemRuns.length > 0 && (
                <>
                  <button
                    ...
                    className="w-full flex items-center gap-2.5 px-4 py-2.5 border-b border-border/50 hover:bg-accent transition-colors text-left"
                  >
                    <svg
                      className={`w-3 h-3 text-muted-foreground/60 transition-transform flex-shrink-0 ${systemExpanded ? "rotate-90" : ""}`}
                    ...
                    <span className="text-xs font-semibold text-muted-foreground/60">System runs</span>
                    <span className="ml-1 inline-flex items-center justify-center w-5 h-5 rounded-full bg-accent border border-border text-[10px] font-bold text-muted-foreground">
                      {systemRuns.length}
                    </span>
                  </button>
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex gap-2">
              <button
                ...
                disabled={offset === 0}
                className="px-3 py-1.5 text-xs bg-card border border-border text-muted-foreground disabled:opacity-40 rounded-lg hover:text-foreground transition-colors"
              >
                Previous
              </button>
              <button
      ...
      {rejectApprovalId !== null && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60">
          <div className="bg-card border border-border rounded-xl p-6 w-full max-w-md space-y-4">
            <h2 className="text-base font-bold text-foreground">Reject Approval</h2>
