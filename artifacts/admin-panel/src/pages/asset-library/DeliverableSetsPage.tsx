...
type ImportRecord = z.infer<typeof DeliverableSetImportSchema>;
...
function JsonImportModal({ onClose, onImported }: { onClose: () => void; onImported: () => void }) {
  ...
  const submit = async () => {
    ...
    records.forEach((rec, idx) => {
      const result = DeliverableSetImportSchema.safeParse(rec);
      if (!result.success) {
    ...
    toast({ title: isBulk ? `${created} created, ${updated} updated` : (validatedRecords[0] && typeof validatedRecords[0].id === "number" ? "Deliverable set updated via import" : "Deliverable set created via import") });
    onImported();
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-lg p-6">
        ...
        <textarea value={raw} onChange={e=>setRaw(e.target.value)} rows={10} spellCheck={false}
          ...
          placeholder={'[{"title": "Set A", "category": "Migration", "deliverables": ["Executive Summary"]}, {"title": "Set B"}]'} />
        {errors.length > 0 && (
    ...
    <div className="p-6 max-w-5xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex gap-2">
          ...
          <button onClick={() => exportAsJson(rows, "deliverable-sets-export.json")} disabled={rows.length === 0} className="flex items-center gap-1.5 px-3 py-2 text-sm border border-gray-200 rounded-lg hover:bg-gray-50 text-gray-600 disabled:opacity-40 disabled:cursor-not-allowed">
