    ...
    <div className="fixed inset-0 z-50 flex justify-end" onClick={e => { if(e.target===e.currentTarget) onClose(); }}>
      <div className="w-full max-w-lg bg-card h-full shadow-2xl flex flex-col">
        <div className="flex items-center justify-between px-6 py-4 border-b">
          <h2 className="font-semibold text-foreground">{isEdit ? "Edit Deliverable Set" : "New Deliverable Set"}</h2>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-5">
          <div>
            ...
            <input value={form.title} onChange={e=>setForm(f=>({...f,title:e.target.value}))}
              className="w-full border border-border rounded px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-primary" placeholder="e.g. M365 Migration Deliverables" />
          </div>
  ...
  return (
    <div className="p-4 sm:p-6 max-w-5xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-xl font-bold text-foreground">Deliverable Sets</h1>
          <p className="text-sm text-muted-foreground mt-0.5">Reusable lists of client-facing deliverables for workflow tasks.</p>
        </div>
        <div className="flex gap-2">
          ...
          <button onClick={() => exportAsJson(rows, "deliverable-sets-export.json")} disabled={rows.length === 0} className="flex items-center gap-1.5 px-3 py-2 text-sm border border-border rounded-lg hover:bg-accent text-muted-foreground disabled:opacity-40 disabled:cursor-not-allowed">
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-border bg-card">
                      ...
                      <th className="px-4 py-3 text-left text-xs font-semibold text-muted-foreground uppercase tracking-wide w-28">Deliverables</th>
                  ...
                  <tbody className="divide-y divide-border">
                    {group.items.map(row => (
                      <tr key={row.id} className="hover:bg-accent/50">
                        ...
                        <td className="px-4 py-3 text-muted-foreground">{row.deliverables.length}</td>
                        ...
                        <td className="px-4 py-3">
                          <div className="flex items-center justify-end gap-1">
                            ...
                            <button onClick={() => exportAsJson(row, `deliverable-set-${row.id}.json`)} className="p-1.5 text-muted-foreground hover:text-primary rounded" title="Export JSON"><Download className="w-3.5 h-3.5"/></button>
