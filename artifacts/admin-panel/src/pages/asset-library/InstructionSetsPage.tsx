        ...
        {items.map((item, i) => (
          <div key={i} className="flex items-start gap-1.5">
            <div className="flex flex-col gap-0.5 pt-1">
              <button type="button" onClick={() => { const a=[...items]; [a[i-1],a[i]]=[a[i],a[i-1]]; onChange(a); }} disabled={i===0} className="text-muted-foreground/60 hover:text-muted-foreground disabled:opacity-30"><ChevronUp className="w-3 h-3"/></button>
              <button type="button" onClick={() => { const a=[...items]; [a[i],a[i+1]]=[a[i+1],a[i]]; onChange(a); }} disabled={i===items.length-1} className="text-muted-foreground/60 hover:text-muted-foreground disabled:opacity-30"><ChevronDown className="w-3 h-3"/></button>
            </div>
            ...
            <textarea
              ...
              rows={2}
              className="flex-1 text-sm border border-border rounded px-2 py-1.5 resize-none focus:outline-none focus:ring-1 focus:ring-primary"
            />
            <button type="button" onClick={() => onChange(items.filter((_,j)=>j!==i))} className="mt-1.5 text-muted-foreground hover:text-red-500"><X className="w-3.5 h-3.5"/></button>
          </div>
        ))}
      </div>
      ...
      {!showNew ? (
        <div className="flex gap-2">
          <select
            ...
            className="flex-1 border border-border rounded px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-primary bg-card"
          >
        ...
        <div className="flex gap-2">
          ...
          />
          <button type="button" onClick={() => void handleCreate()} disabled={createMutation.isPending || !newName.trim()} className="px-3 py-2 bg-primary text-white text-sm rounded hover:bg-[#005fa3] disabled:opacity-50">
    ...
    <div className="fixed inset-0 z-50 flex justify-end" onClick={e => { if(e.target===e.currentTarget) onClose(); }}>
      <div className="w-full max-w-lg bg-card h-full shadow-2xl flex flex-col">
        <div className="flex items-center justify-between px-6 py-4 border-b">
          <h2 className="font-semibold text-foreground">{isEdit ? "Edit Instruction Set" : "New Instruction Set"}</h2>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-5">
          <div>
            ...
            <input value={form.title} onChange={e=>setForm(f=>({...f,title:e.target.value}))}
              className="w-full border border-border rounded px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-primary" placeholder="e.g. Copilot Governance Checklist" />
          </div>
          <div>
            <label className="block text-xs font-semibold text-foreground/90 uppercase tracking-wide mb-1">Description</label>
            <textarea value={form.description} onChange={e=>setForm(f=>({...f,description:e.target.value}))} rows={2}
              className="w-full border border-border rounded px-3 py-2 text-sm resize-none focus:outline-none focus:ring-1 focus:ring-primary" placeholder="Optional description" />
          </div>
  ...
  return (
    <div className="p-4 sm:p-6 max-w-5xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-xl font-bold text-foreground">Instruction Sets</h1>
          <p className="text-sm text-muted-foreground mt-0.5">Reusable ordered instruction lists for workflow tasks.</p>
        </div>
        <div className="flex gap-2">
          ...
          <button onClick={() => exportAsJson(rows, "instruction-sets-export.json")} disabled={rows.length === 0} className="flex items-center gap-1.5 px-3 py-2 text-sm border border-border rounded-lg hover:bg-accent text-muted-foreground disabled:opacity-40 disabled:cursor-not-allowed">
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-border bg-card">
                      ...
                      <th className="px-4 py-3 text-left text-xs font-semibold text-muted-foreground uppercase tracking-wide w-24">Steps</th>
                    ...
                    {group.items.map(row => (
                      <tr key={row.id} className="hover:bg-accent/50">
                        ...
                        <td className="px-4 py-3">
                          ...
                          {row.description && <div className="text-xs text-muted-foreground mt-0.5 truncate max-w-sm">{row.description}</div>}
                        </td>
                        <td className="px-4 py-3 text-muted-foreground">{row.instructions.length}</td>
                        <td className="px-4 py-3 text-muted-foreground text-xs">{new Date(row.createdAt).toLocaleDateString()}</td>
                        <td className="px-4 py-3">
                          <div className="flex items-center justify-end gap-1">
                            <button onClick={() => setEditorRecord(row)} className="p-1.5 text-muted-foreground hover:text-primary rounded" title="Edit">
                              <Pencil className="w-3.5 h-3.5"/>
                            </button>
                            <button onClick={() => exportAsJson(row, `instruction-set-${row.id}.json`)} className="p-1.5 text-muted-foreground hover:text-primary rounded" title="Export JSON">
                              <Download className="w-3.5 h-3.5"/>
                            </button>
                            <button onClick={() => setDeleteTarget(row)} className="p-1.5 text-muted-foreground hover:text-red-500 rounded" title="Delete">
