...
function JsonImportModal({ onClose, onImported }: { onClose: () => void; onImported: () => void }) {
  ...
  const submit = async () => {
    ...
    const validated = ChecklistImportSchema.safeParse(parsed);
    ...
    try {
      ...
      toast({ title: hasId ? "Checklist updated via import" : "Checklist created via import" });
      onImported();
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-lg p-6">
        ...
        <p className="text-sm text-gray-500 mb-3">Paste a full checklist JSON. If it has an <code className="bg-gray-100 px-1 rounded">id</code> field the record will be updated; otherwise a new one is created.</p>
        <textarea value={raw} onChange={e=>setRaw(e.target.value)} rows={10} spellCheck={false}
          ...
          placeholder='{"title": "My Checklist", "items": [{"id": "1", "label": "Item 1"}]}' />
        {errors.length > 0 && (
