    ...
    <div className="fixed inset-0 z-50 flex justify-end" onClick={e => { if(e.target===e.currentTarget) onClose(); }}>
      <div className="w-full max-w-lg bg-[#161B22] h-full shadow-2xl flex flex-col">
        <div className="flex items-center justify-between px-6 py-4 border-b">
          <h2 className="font-semibold text-[#E6EDF3]">{isEdit ? "Edit Artifact Set" : "New Artifact Set"}</h2>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-5">
          <div>
            ...
            <input value={form.title} onChange={e=>setForm(f=>({...f,title:e.target.value}))}
              className="w-full border border-[#30363D] rounded px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-[#0078D4]" placeholder="e.g. Governance Report Artifacts" />
          </div>
    ...
    <div className="p-6 max-w-5xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-[#7D8590] mt-0.5">Reusable lists of files and deliverables produced by workflow tasks.</p>
        </div>
        <div className="flex gap-2">
          ...
          <button onClick={() => exportAsJson(rows, "artifact-sets-export.json")} disabled={rows.length === 0} className="flex items-center gap-1.5 px-3 py-2 text-sm border border-[#30363D] rounded-lg hover:bg-[#1C2128] text-[#7D8590] disabled:opacity-40 disabled:cursor-not-allowed">
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-[#30363D] bg-[#161B22]">
                      ...
                      <th className="px-4 py-3 text-left text-xs font-semibold text-[#7D8590] uppercase tracking-wide w-24">Artifacts</th>
                  ...
                  <tbody className="divide-y divide-gray-50">
                    {group.items.map(row => (
                      <tr key={row.id} className="hover:bg-[#1C2128]/50">
                        ...
                        <td className="px-4 py-3 text-[#7D8590]">{row.artifacts.length}</td>
                        ...
                        <td className="px-4 py-3">
                          <div className="flex items-center justify-end gap-1">
                            ...
                            <button onClick={() => exportAsJson(row, `artifact-set-${row.id}.json`)} className="p-1.5 text-[#7D8590] hover:text-[#0078D4] rounded" title="Export JSON"><Download className="w-3.5 h-3.5"/></button>
