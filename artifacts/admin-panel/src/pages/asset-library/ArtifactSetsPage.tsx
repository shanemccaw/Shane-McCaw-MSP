    ...
    <div className="fixed inset-0 z-50 flex justify-end" onClick={e => { if(e.target===e.currentTarget) onClose(); }}>
      <div className="w-full max-w-lg bg-white h-full shadow-2xl flex flex-col">
        <div className="flex items-center justify-between px-6 py-4 border-b">
          <h2 className="font-semibold text-gray-900">{isEdit ? "Edit Artifact Set" : "New Artifact Set"}</h2>
...
function JsonImportModal({ onClose, onImported }: { onClose: () => void; onImported: () => void }) {
  ...
  const submit = async () => {
    ...
    const validated = ArtifactSetImportSchema.safeParse(parsed);
    ...
    try {
      ...
      toast({ title: hasId ? "Artifact set updated via import" : "Artifact set created via import" });
      onImported();
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-lg p-6">
        ...
        <p className="text-sm text-gray-500 mb-3">Paste a full artifact set JSON. If it has an <code className="bg-gray-100 px-1 rounded">id</code> field the record will be updated; otherwise a new one is created.</p>
        <textarea value={raw} onChange={e=>setRaw(e.target.value)} rows={10} spellCheck={false}
          ...
          placeholder='{"title": "My Artifacts", "artifacts": ["report.pdf", "slides.pptx"]}' />
        {errors.length > 0 && (
  ...
  return (
    <div className="p-6 max-w-5xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-xl font-bold text-gray-900">Artifact Sets</h1>
          <p className="text-sm text-gray-500 mt-0.5">Reusable lists of files and deliverables produced by workflow tasks.</p>
        </div>
        <div className="flex gap-2">
