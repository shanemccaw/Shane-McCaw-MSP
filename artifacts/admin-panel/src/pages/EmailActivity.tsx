...
function SkeletonDetail() {
  return (
    <div className="p-6 space-y-4 animate-pulse">
      ...
      <div className="mt-6 h-40 bg-accent rounded-xl" />
    </div>
  );
}
...
function EmailListPanel({
  ...
  return (
    <div className="flex flex-col h-full border-r border-border bg-card">
              ...
              return (
                <button
                  ...
                  className={`w-full text-left px-4 py-3 flex gap-3 transition-colors ${
                    active ? "bg-primary/10 border-l-2 border-primary" : "hover:bg-accent/50 border-l-2 border-transparent"
                  }`}
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between gap-2">
                      ...
                      <span className="text-[10px] text-muted-foreground shrink-0 whitespace-nowrap">{timeAgo(row.email.receivedAt)}</span>
                    </div>
                    ...
                    <div className="mt-1.5 flex items-center gap-1.5">
                      {linked ? (
                        <span className="inline-flex items-center px-1.5 py-0.5 rounded-full text-[9px] font-bold bg-primary/100/15 text-blue-400 max-w-[130px] truncate">
      ...
      {/* Pagination */}
      {totalPages > 1 && (
        <div className="shrink-0 px-4 py-2.5 border-t border-border flex items-center justify-between">
          <span className="text-[10px] text-muted-foreground">{total} total</span>
          <div className="flex items-center gap-1.5">
            <button
              onClick={() => onPageChange(Math.max(1, page - 1))}
              disabled={page === 1}
              className="w-6 h-6 flex items-center justify-center rounded border border-border text-muted-foreground text-xs hover:bg-accent disabled:opacity-40"
            >
              ‹
            </button>
  ...
  return (
    <div className="flex flex-col h-full overflow-y-auto">
      ...
      <div className="px-6 pt-5 pb-4 border-b border-border shrink-0">
        ...
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-xs text-muted-foreground">{detail.email.senderAddress}</p>
            <p className="text-xs text-muted-foreground mt-0.5">{formatDateLong(detail.email.receivedAt)}</p>
          </div>
          {/* Assign dropdown */}
          <div className="shrink-0">
            <select
              ...
              className="text-xs border border-border rounded-lg px-2 py-1.5 text-foreground/90 bg-card focus:outline-none focus:ring-1 focus:ring-primary disabled:opacity-50 max-w-[180px]"
            >
      ...
      <div className="px-6 py-4 border-b border-border shrink-0">
        {detail.bodyContentType === "html" && detail.bodyContent ? (
          <iframe
            ...
            className="w-full min-h-[280px] border-0 rounded-lg bg-card"
            title="Email body"
            style={{ height: "320px" }}
          />
        ) : detail.bodyContent ? (
          <pre className="text-sm text-foreground/90 whitespace-pre-wrap font-sans leading-relaxed max-h-72 overflow-y-auto">
            {detail.bodyContent}
          </pre>
        ) : (
      ...
      {detail.clientId ? (
        <div className="px-6 py-4 border-b border-border shrink-0">
          ...
          <div className="flex items-center gap-3 p-3 bg-card rounded-xl">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-sm font-semibold text-foreground">{detail.clientName ?? detail.clientEmail}</p>
              {detail.clientCompany && <p className="text-xs text-muted-foreground">{detail.clientCompany}</p>}
              {detail.clientEmail && <p className="text-xs text-muted-foreground">{detail.clientEmail}</p>}
            </div>
            <button
              onClick={() => navigate(`/crm/clients/${detail.clientId}`)}
              className="flex-shrink-0 text-[10px] font-semibold text-primary hover:underline"
            ...
            {linkMode === "project" ? (
              <>
                {detail.linkedProjectTitle && (
                  <div className="mb-2 flex items-center gap-2 px-3 py-2 bg-primary/100/10 border border-blue-500/20 rounded-xl">
                    ...
                    <span className="text-xs font-semibold text-primary truncate flex-1">{detail.linkedProjectTitle}</span>
                    <a
                ...
                {projectsLoading ? (
                  ...
                  <p className="text-xs text-muted-foreground">No active projects for this client.</p>
                ) : (
                  <div className="flex gap-2">
                    <select
                      value={selectedProjectId}
                      onChange={e => setSelectedProjectId(e.target.value)}
                      className="flex-1 border border-border rounded-lg px-2 py-1.5 text-xs text-foreground/90 focus:outline-none focus:ring-1 focus:ring-primary"
                    >
                    ...
                    <button
                      ...
                      disabled={linkSaving}
                      className="px-3 py-1.5 bg-primary text-white text-xs font-semibold rounded-lg hover:bg-[#005fa3] disabled:opacity-50 transition-colors shrink-0"
                    >
                      {linkSaving ? "…" : "Save"}
                    </button>
                  </div>
        ...
        <div className="px-6 py-4 border-b border-border shrink-0">
          ...
          {/* Lead picker */}
          <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest mb-2">Linked Lead</p>
          {detail.linkedLeadName && (
            <div className="mb-2 flex items-center gap-2 px-3 py-2 bg-purple-50 border border-purple-100 rounded-xl">
              <svg className="w-3.5 h-3.5 text-purple-600 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                <path strokeLinecap="round" strokeLinejoin="round" d="M15.75 6a3.75 3.75 0 11-7.5 0 3.75 3.75 0 017.5 0zM4.501 20.118a7.5 7.5 0 0114.998 0A17.933 17.933 0 0112 21.75c-2.676 0-5.216-.584-7.499-1.632z" />
      ...
      <div className="px-6 py-4 shrink-0">
        <div className="flex items-center justify-between mb-2">
          <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest">Next Step</p>
          {!nextStepOpen && !createdTask && detail.clientId && (
            <button
              onClick={() => setNextStepOpen(true)}
              className="text-xs text-primary hover:text-[#005fa3] font-semibold transition-colors flex items-center gap-1"
            >
        ...
        {createdTask ? (
          ...
          <form onSubmit={e => void handleCreateTask(e)} className="p-4 bg-card border border-border rounded-xl space-y-3">
            <div>
              <label className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest block mb-1">Task title</label>
              <input
                type="text"
                value={taskTitle}
                onChange={e => setTaskTitle(e.target.value)}
                placeholder="e.g. Follow up on proposal"
                className="w-full border border-border rounded-lg px-3 py-2 text-sm text-foreground focus:outline-none focus:ring-1 focus:ring-primary"
                required
              />
            </div>
            <div>
              <label className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest block mb-1">Project</label>
              {activeProjects.length === 0 ? (
                <p className="text-xs text-muted-foreground italic">No active projects for this client.</p>
              ) : (
                <select
                  value={taskProjectId}
                  onChange={e => setTaskProjectId(e.target.value)}
                  className="w-full border border-border rounded-lg px-3 py-2 text-sm text-foreground/90 focus:outline-none focus:ring-1 focus:ring-primary"
                  required
                >
            ...
            <div className="flex gap-3">
              <div className="flex-1">
                <label className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest block mb-1">Priority</label>
                <select
                  value={taskPriority}
              ...
              <div className="flex-1">
                <label className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest block mb-1">Due date</label>
                <input
                  type="date"
                  value={taskDueDate}
            ...
            <div>
              <label className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest block mb-1">Notes (optional)</label>
              <textarea
                value={taskNotes}
                onChange={e => setTaskNotes(e.target.value)}
                rows={2}
                placeholder="Additional context…"
                className="w-full border border-border rounded-lg px-3 py-2 text-sm text-foreground focus:outline-none focus:ring-1 focus:ring-primary resize-none"
              />
            </div>
            ...
            <div className="flex gap-2 justify-end">
              ...
              <button
                ...
                className="px-4 py-1.5 bg-primary text-white text-xs font-semibold rounded-lg hover:bg-[#005fa3] disabled:opacity-50 transition-colors"
              >
          ...
          <p className="text-xs text-muted-foreground italic">Link a client above to add follow-up tasks.</p>
        ) : (
          <p className="text-xs text-muted-foreground italic">No follow-up tasks yet.</p>
        )}
      </div>
    </div>
  );
    ...
    <div className="border-t border-border bg-card shrink-0">
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs font-semibold text-muted-foreground">Email Settings</span>
        </div>
        <svg
      ...
      {open && (
        <div className="border-t border-border">
          ...
          <div className="px-4 py-3">
            <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest mb-2">Matching Rules</p>
            ...
            {rulesLoading ? (
              ...
              <p className="text-xs text-muted-foreground mb-3">No rules defined yet.</p>
            ) : (
              <div className="space-y-1.5 mb-3">
                {rules.map(row => (
                  <div key={row.rule.id} className="flex items-center gap-2 py-1.5 px-3 bg-card rounded-lg">
                    ...
                    <span className="font-mono text-xs text-foreground flex-1 truncate">{ruleLabel(row.rule.domain)}</span>
                    <span className="text-xs text-muted-foreground truncate max-w-[100px]">{row.clientName ?? row.clientEmail}</span>
                    <button
            ...
            <form onSubmit={e => void addRule(e)} className="space-y-2">
              <input
                ...
                className="w-full border border-border rounded-lg px-3 py-1.5 text-xs text-foreground focus:outline-none focus:ring-1 focus:ring-primary"
              />
              <div className="flex gap-2">
                <select
                  value={newRuleUserId}
                ...
                <button
                  ...
                  className="px-3 py-1.5 bg-primary text-white text-xs font-semibold rounded-lg hover:bg-[#005fa3] disabled:opacity-50 transition-colors"
                >
                  {addingRule ? "…" : "Add"}
                </button>
              </div>
          ...
          <div className="px-4 py-3 border-t border-border bg-primary/10">
            <p className="text-[10px] font-bold text-primary uppercase tracking-widest mb-1.5">Connect M365 Mailbox</p>
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-48px)]">
      ...
      <div className="px-6 py-4 border-b border-border bg-card shrink-0">
        <h1 className="text-lg font-bold text-foreground">Email Activity</h1>
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div className="w-[340px] shrink-0 flex flex-col overflow-hidden border-r border-border">
