...
export default function PromptCenter() {
  ...
  return (
    <div className="p-6 max-w-6xl mx-auto space-y-6">
      <div>
        <h1 className="text-xl font-bold text-foreground">AI Prompt Center</h1>
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full pl-9 pr-3 py-2 bg-accent border border-border rounded-lg text-sm text-foreground placeholder:text-muted-foreground/60 focus:outline-none focus:border-primary"
          />
        </div>
            ...
            <button
              ...
              className={`px-3 py-1.5 rounded-lg text-xs font-medium border transition-colors capitalize ${
                categoryFilter === cat
                  ...
                  : "text-muted-foreground border-border hover:border-muted-foreground/60 hover:text-foreground/90"
              }`}
            >
              {cat}
            </button>
      ...
      <div className="text-xs text-muted-foreground">{filtered.length} prompt{filtered.length !== 1 ? "s" : ""}</div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-border">
                <th className="text-left px-4 py-3 text-xs font-semibold text-muted-foreground uppercase tracking-wider">Prompt</th>
                ...
                <th className="text-left px-4 py-3 text-xs font-semibold text-muted-foreground uppercase tracking-wider hidden lg:table-cell">Feature Area</th>
                <th className="text-left px-4 py-3 text-xs font-semibold text-muted-foreground uppercase tracking-wider hidden xl:table-cell">Status</th>
                <th className="px-4 py-3" />
              </tr>
            </thead>
            <tbody className="divide-y divide-accent">
              {filtered.map((prompt) => (
                <tr key={prompt.id} className="hover:bg-accent transition-colors group">
                  <td className="px-4 py-3">
                    <div className="font-medium text-foreground">{prompt.name}</div>
                    <div className="text-xs text-muted-foreground/60 mt-0.5 hidden sm:block">{prompt.description.slice(0, 80)}{prompt.description.length > 80 ? "…" : ""}</div>
                  </td>
                  ...
                  <td className="px-4 py-3 hidden lg:table-cell">
                    <div className="flex items-center gap-1.5 text-xs text-muted-foreground">
                      {prompt.featureArea}
                      {prompt.featureRoute && (
                        <a href={`/admin-panel${prompt.featureRoute}`} className="text-primary hover:text-[#006CBE] opacity-0 group-hover:opacity-100 transition-opacity" title="Go to feature">
                  ...
                  <td className="px-4 py-3 text-right">
                    <Link href={`/prompt-center/${prompt.id}`}>
                      <span className="inline-flex items-center gap-1 text-xs text-primary hover:text-[#006CBE] font-medium cursor-pointer">
                        Edit
                        <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
