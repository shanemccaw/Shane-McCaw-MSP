...
function SortBtn({ col, sortCol, sortDir, onSort }: { col: string; sortCol: string; sortDir: SortDir; onSort: (c: string) => void }) {
  ...
  return (
    <button onClick={() => onSort(col)} className="inline-flex items-center gap-0.5 group">
      <span className={active ? "text-primary" : ""}>{col}</span>
      <svg className={`w-3 h-3 ml-0.5 transition-transform ${active && sortDir === "asc" ? "rotate-180" : ""} ${active ? "text-primary" : "text-muted-foreground/60 group-hover:text-muted-foreground"}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={3}>
...
export default function AnalyticsPage() {
  ...
  return (
    <div className="p-4 sm:p-6 max-w-[1280px] space-y-6">
            ...
            {isCustom && (
              <div className="flex items-center gap-1.5 bg-card border border-border rounded-lg px-2 py-1 shadow-sm">
                ...
                <button
                  ...
                  className="ml-1 px-2 py-0.5 text-[10px] font-bold bg-primary text-white rounded hover:bg-[#005A9E] disabled:opacity-40 transition-colors"
                >
                  Apply
                </button>
              </div>
      ...
      {/* Tab Bar */}
      <div className="flex items-center border-b border-border -mt-2 mb-2">
        {([
      ...
      {kpisLoading ? (
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          {[
            ...
            <div key={card.label} className="bg-card border border-border rounded-xl p-5 flex items-start gap-4 shadow-sm">
      ...
      <section className="bg-card border border-border rounded-xl p-5 shadow-sm">
        <h2 className="text-sm font-bold text-foreground uppercase tracking-widest mb-4">Page Views Over Time</h2>
        {seriesLoading ? (
          <div className="h-56 bg-card rounded-lg animate-pulse" />
        ) : !series || series.length === 0 ? (
          <p className="text-sm text-muted-foreground text-center py-16">No data yet — traffic will appear here once visitors arrive.</p>
        ) : (
          <ResponsiveContainer width="100%" height={220}>
            <LineChart data={series} margin={{ top: 4, right: 8, left: -16, bottom: 0 }}>
              ...
              <XAxis dataKey="date" tick={{ fontSize: 10, fill: "#8B94A3" }} axisLine={false} tickLine={false}
                tickFormatter={(d: string) => { const [, m, day] = d.split("-"); return `${m}/${day}`; }}
              />
              <YAxis tick={{ fontSize: 10, fill: "#8B94A3" }} axisLine={false} tickLine={false} allowDecimals={false} />
              <RechartsTooltip
                contentStyle={{ fontSize: 12, borderRadius: 8, border: "1px solid #232A36", background: "#171C26", color: "#E6EDF3" }}
                formatter={(v: number) => [v, "Views"]}
              />
              <Line type="monotone" dataKey="views" stroke="#2F6FED" strokeWidth={2} dot={false} activeDot={{ r: 4 }} />
            </LineChart>
          </ResponsiveContainer>
        )}
      </section>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {/* Top Pages — sortable */}
        <section className="bg-card border border-border rounded-xl p-5 shadow-sm">
          <h2 className="text-sm font-bold text-foreground uppercase tracking-widest mb-4">Top Pages</h2>
          {topPagesLoading ? (
            <div className="space-y-2">{Array.from({ length: 6 }).map((_, i) => <div key={i} className="h-8 bg-card rounded-lg animate-pulse" />)}</div>
          ) : !topPages || topPages.length === 0 ? (
            <p className="text-xs text-muted-foreground text-center py-8">No page view data yet.</p>
          ) : (
                ...
                <tbody>
                  {sortedPages().map((row, i) => (
                    <tr key={i} className="border-b border-border hover:bg-accent transition-colors">
                      <td className="py-2 pr-3 text-foreground font-medium truncate max-w-[180px]" title={row.page}>{row.page || "/"}</td>
                      <td className="py-2 pr-3 text-right text-muted-foreground font-semibold">{fmt(row.views)}</td>
                      <td className="py-2 pr-3 text-right text-muted-foreground">{row.avgDuration ? fmtTime(row.avgDuration) : "—"}</td>
                      <td className="py-2 text-right">
                        <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded-full ${row.bounceRate > 70 ? "bg-red-500/15 text-red-400" : "bg-border/50 text-muted-foreground"}`}>
                          {row.bounceRate}%
                        </span>
                      </td>
                    </tr>
      ...
      <section className="bg-card border border-border rounded-xl p-5 shadow-sm">
        <div className="flex items-center justify-between flex-wrap gap-3 mb-4">
          <div>
            <h2 className="text-sm font-bold text-foreground uppercase tracking-widest">12-Month Revenue Forecast</h2>
            <p className="text-[10px] text-muted-foreground mt-0.5">AI-powered forecast with confidence bands based on historical invoices &amp; MRR</p>
          </div>
          <div className="flex items-center gap-2">
            {revForecast?.generatedAt && (
              <span className="text-[10px] text-muted-foreground/60">Updated {new Date(revForecast.generatedAt).toLocaleDateString("en-US", { month: "short", day: "numeric" })}</span>
            )}
            <button
        ...
        {revLoading ? (
          <div className="h-64 bg-accent rounded-xl animate-pulse" />
          ...
          <div className="flex flex-col items-center justify-center py-16 text-center">
            ...
            <p className="text-xs text-muted-foreground mt-1 max-w-[260px]">Click Generate Forecast to have Claude analyze your revenue history and predict the next 12 months.</p>
          </div>
        ) : (
          <>
            {revForecast.narrative && (
              <div className="bg-primary/10 border border-primary/20 rounded-xl px-4 py-3 mb-5 flex items-start gap-2">
                ...
                <p className="text-xs text-foreground/90 leading-relaxed">{revForecast.narrative}</p>
              </div>
            )}
            ...
            <ResponsiveContainer width="100%" height={280}>
              <ComposedChart data={revForecast.rows} margin={{ top: 4, right: 8, left: 8, bottom: 0 }}>
                ...
                <Area type="monotone" dataKey="forecast" stroke="#2F6FED" fill="url(#forecastGrad)" strokeWidth={2} name="forecast" dot={false} />
                <Area type="monotone" dataKey="lowerBound" stroke="transparent" fill="transparent" strokeWidth={0} name="lowerBound" />
                <ReferenceLine y={revForecast.rows[0]?.forecast ?? 0} stroke="#8B94A3" strokeDasharray="4 4" strokeWidth={1} />
              </ComposedChart>
            </ResponsiveContainer>

            {/* Full 12-month table */}
            <div className="mt-6 pt-5 border-t border-border">
              <h3 className="text-xs font-bold text-muted-foreground uppercase tracking-widest mb-3">Monthly Breakdown — All 12 Months</h3>
              <div className="overflow-x-auto">
                <table className="w-full text-xs">
                  <thead>
                    <tr className="border-b border-border">
                      <th className="text-left py-2 pr-4 font-semibold text-muted-foreground uppercase tracking-widest text-[10px]">Month</th>
                      <th className="text-right py-2 pr-4 font-semibold text-muted-foreground uppercase tracking-widest text-[10px]">Forecast</th>
                      <th className="text-right py-2 pr-4 font-semibold text-muted-foreground uppercase tracking-widest text-[10px]">Lower Bound</th>
                      <th className="text-right py-2 pr-4 font-semibold text-muted-foreground uppercase tracking-widest text-[10px]">Upper Bound</th>
                      <th className="text-right py-2 font-semibold text-muted-foreground uppercase tracking-widest text-[10px]">Range</th>
                    </tr>
                  </thead>
                  <tbody>
                    {revForecast.rows.map((r, i) => {
                      ...
                      const isFirst = i === 0;
                      return (
                        <tr key={r.period} className={`border-b border-border hover:bg-accent transition-colors ${isFirst ? "bg-primary/5" : ""}`}>
                          <td className="py-2.5 pr-4 font-medium text-foreground">
                            {isFirst && <span className="text-[9px] font-bold text-primary bg-primary/10 px-1 py-0.5 rounded mr-1.5">Next</span>}
                            {label}
                          </td>
                          <td className="py-2.5 pr-4 text-right font-bold text-foreground">{fmtUsd(r.forecast)}</td>
                          <td className="py-2.5 pr-4 text-right text-muted-foreground">{fmtUsd(r.lowerBound)}</td>
                          <td className="py-2.5 pr-4 text-right text-muted-foreground">{fmtUsd(r.upperBound)}</td>
                          <td className="py-2.5 text-right text-muted-foreground/60 text-[10px]">±{fmtUsd(range / 2)}</td>
                        </tr>
                      );
                    })}
                  </tbody>
                  <tfoot>
                    <tr className="border-t border-border">
                      <td className="pt-3 pr-4 text-[10px] font-bold text-muted-foreground uppercase tracking-widest">12-Month Total</td>
                      <td className="pt-3 pr-4 text-right font-bold text-foreground">{fmtUsd(revForecast.rows.reduce((s, r) => s + r.forecast, 0))}</td>
                      <td className="pt-3 pr-4 text-right text-muted-foreground">{fmtUsd(revForecast.rows.reduce((s, r) => s + r.lowerBound, 0))}</td>
                      <td className="pt-3 pr-4 text-right text-muted-foreground">{fmtUsd(revForecast.rows.reduce((s, r) => s + r.upperBound, 0))}</td>
                      <td />
                    </tr>
                  </tfoot>
                </table>
        ...
        <section className="bg-card border border-border rounded-xl p-5 shadow-sm">
          ...
          {cardClicksLoading ? (
            <div className="space-y-3 mt-5">
              {Array.from({ length: 4 }).map((_, i) => <div key={i} className="h-10 bg-accent rounded-lg animate-pulse" />)}
            </div>
          ) : cardClicksError ? (
            ...
            <>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mt-4 mb-6">
                {cardClicks.map((row, i) => {
                  const COLORS = ["#2F6FED", "#00B4D8", "#7C3AED", "#10B981"];
              ...
              <div className="mb-6 mt-2">
                ...
                {cardClickTrendLoading ? (
                  ...
                  <p className="text-xs text-muted-foreground text-center py-10">Not enough data to show a trend yet — data will appear here once multiple {cardClickTrend?.granularity === "month" ? "months" : "weeks"} have card-click events.</p>
                ) : (
                  (() => {
                    const COLORS = ["#2F6FED", "#00B4D8", "#7C3AED", "#10B981", "#F59E0B", "#EF4444"];
                    ...
                    return (
                      <ResponsiveContainer width="100%" height={220}>
                        ...
                        >
                          ...
                          />
                          <Legend
                            wrapperStyle={{ fontSize: 11, color: "#8B94A3", paddingTop: 8 }}
                            iconType="circle"
                            iconSize={8}
                          />
                ...
                <table className="w-full text-xs">
                  ...
                  <tbody>
                    {cardClicks.map((row, i) => {
                      ...
                      const color = COLORS[i % COLORS.length];
                      return (
                        <tr key={row.cardName} className="border-b border-border hover:bg-accent transition-colors">
