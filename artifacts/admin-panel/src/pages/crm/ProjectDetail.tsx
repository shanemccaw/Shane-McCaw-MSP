...
export default function ProjectDetailPage() {
  ...
  const [dismissedAutoFireIds, setDismissedAutoFireIds] = useState<Set<number>>(new Set());
  const [retryingTaskIds, setRetryingTaskIds] = useState<Set<number>>(new Set());

  const exhaustedCards = tasks.filter(
    t => (t.completionStatus === "auto_fire_exhausted" || t.completionStatus === "auto_fire_failed")
      && !dismissedAutoFireIds.has(t.id)
  );

  const handleRetryAutoFire = async (taskId: number) => {
    setRetryingTaskIds(prev => new Set(prev).add(taskId));
    try {
      const res = await fetchWithAuth(`/api/admin/kanban-tasks/${taskId}/retry-auto-fire`, { method: "POST" });
      if (!res.ok) {
        const data = await res.json() as { error?: string };
        toast({ title: "Retry failed", description: data.error ?? "Could not retry auto-fire.", variant: "destructive" });
      } else {
        const updated = await res.json() as KanbanTask;
        handleTasksChange(ts => ts.map(t => t.id === taskId ? updated : t));
        toast({ title: "Auto-fire retried", description: "Card moved back to backlog and will fire automatically." });
      }
    } catch {
      toast({ title: "Retry failed", description: "Network error. Please try again.", variant: "destructive" });
    } finally {
      setRetryingTaskIds(prev => { const s = new Set(prev); s.delete(taskId); return s; });
    }
  };
    ...
    <div className="p-4 sm:p-6 max-w-[1400px]">
      ...
      <section className="mb-8">
        ...
        {exhaustedCards.length > 0 && (
          <div className="mb-4 rounded-xl border border-red-500/40 bg-red-500/10 px-4 py-3.5">
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1 min-w-0">
                <p className="text-sm font-bold text-red-400 mb-0.5">
                  Auto-fire failed on {exhaustedCards.length} {exhaustedCards.length === 1 ? "card" : "cards"}
                </p>
                <p className="text-xs text-red-400/70 mb-3">
                  The following {exhaustedCards.length === 1 ? "card has" : "cards have"} exhausted all auto-fire retries and need attention.
                </p>
                  ...
                  {exhaustedCards.map(card => {
                    ...
                    return (
                      <div key={card.id} className="flex items-center justify-between gap-3 bg-red-500/10 border border-red-500/20 rounded-lg px-3 py-2">
                        ...
                        <button
                          disabled={isRetrying}
                          onClick={() => void handleRetryAutoFire(card.id)}
                          className="flex-shrink-0 flex items-center gap-1.5 text-[11px] font-bold px-2.5 py-1.5 rounded-lg bg-red-500/20 text-red-300 hover:bg-red-500/30 disabled:opacity-50 transition-colors border border-red-500/30"
                        >
                          {isRetrying ? (
                            <div className="w-3 h-3 border border-red-300/40 border-t-red-300 rounded-full animate-spin" />
                          ) : (
              ...
              <button
                onClick={() => setDismissedAutoFireIds(prev => new Set([...prev, ...exhaustedCards.map(c => c.id)]))}
                className="flex-shrink-0 text-red-400/60 hover:text-red-400 transition-colors mt-0.5"
