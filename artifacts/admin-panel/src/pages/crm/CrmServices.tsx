  ...
  return (
    <div className="p-6 max-w-[1200px]">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-xl font-bold text-[#0A2540]">CRM Services</h1>
          <p className="text-sm text-muted-foreground mt-0.5">Define service templates and assign them to clients.</p>
        </div>
      ...
      {showForm && (
        <div className="bg-[#F7F9FC] border border-border rounded-xl p-5 mb-6">
          ...
          <form onSubmit={handleSubmit} className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-xs font-semibold text-[#0A2540] mb-1">Fixed Price (USD)</label>
              <input type="text" placeholder="e.g. 497" value={form.price} onChange={e => setForm(f => ({ ...f, price: e.target.value }))}
                className="w-full border border-border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#0078D4]" />
            </div>
            <div>
              <label className="block text-xs font-semibold text-[#0A2540] mb-1">Base Price (USD)</label>
              <input type="text" placeholder="e.g. 1000" value={form.basePrice} onChange={e => setForm(f => ({ ...f, basePrice: e.target.value }))}
                className="w-full border border-border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#0078D4]" />
            </div>
            <div>
              <label className="block text-xs font-semibold text-[#0A2540] mb-1">Max Price (USD)</label>
              <input type="text" placeholder="e.g. 5000" value={form.maxPrice} onChange={e => setForm(f => ({ ...f, maxPrice: e.target.value }))}
                className="w-full border border-border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#0078D4]" />
            </div>
            <div>
            ...
            {(["description", "deliverables"] as const).map(key => (
              ...
              <div className="bg-white border border-border rounded-xl px-5 py-4 flex items-center gap-4">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 flex-wrap">
                    <p className="font-semibold text-[#0A2540]">{s.name}</p>
                    {s.category && <span className="text-xs bg-[#F7F9FC] border border-border rounded px-2 py-0.5 text-muted-foreground">{s.category}</span>}
                  </div>
                  <div className="flex items-center gap-3 mt-0.5 text-xs text-muted-foreground">
                    {s.price && <span className="font-semibold text-[#0A2540]">${s.price}</span>}
                    {s.basePrice && !s.price && <span>From ${s.basePrice}{s.maxPrice ? `–$${s.maxPrice}` : ""}</span>}
                    {s.durationDays && <span>{s.durationDays}d</span>}
                    {s.orderWorkflow && s.orderWorkflow.length > 0 && (
                      <span className="text-[#0078D4] font-semibold">{s.orderWorkflow.length} wizard step{s.orderWorkflow.length !== 1 ? "s" : ""}</span>
                    )}
                  </div>
                </div>
                <div className="flex items-center gap-2 flex-shrink-0">
                  <button onClick={() => toggleWorkflow(s.id)}
                    className={`text-xs font-semibold px-3 py-1.5 rounded-lg border transition-colors ${workflowServiceId === s.id ? "bg-[#0078D4] text-white border-[#0078D4]" : "border-border text-muted-foreground hover:border-[#0078D4] hover:text-[#0078D4]"}`}>
                    Wizard
                  </button>
                  <button onClick={() => handleEdit(s)} className="text-xs font-semibold text-[#0078D4] hover:underline">Edit</button>
                </div>
              </div>
            </div>
          ))}
