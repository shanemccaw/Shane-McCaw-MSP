...
function ProjectTypeBadge({ type }: { type: string }) {
  const map: Record<string, string> = {
    ...
    "micro-offer": "bg-teal-100 text-teal-700",
  };
...
export default function TestimonialsPage() {
  ...
  return (
    <div className="p-6 max-w-4xl">
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-[#0A2540]">Testimonials</h1>
      ...
      {loading ? (
        ...
        <div className="bg-white border border-border rounded-xl p-12 text-center">
          ...
          <p className="text-sm font-semibold text-[#0A2540]">No signed closures yet</p>
          ...
          {published.length > 0 && (
            <section>
              ...
              <div className="space-y-4">
                {published.map(item => (
                  <div key={item.id} className="bg-white border border-border rounded-xl p-5 space-y-3">
                    <div className="flex items-start justify-between gap-3 flex-wrap">
                      <div className="space-y-1">
                        <div className="flex items-center gap-2 flex-wrap">
                          <p className="text-sm font-bold text-[#0A2540]">{item.projectTitle}</p>
          ...
          {signedOff.length > 0 && (
            <section>
              ...
              <div className="space-y-3">
                {signedOff.map(item => (
                  <div key={item.id} className="bg-[#F7F9FC] border border-border rounded-xl p-4 flex items-center justify-between gap-4 flex-wrap">
                    <div className="space-y-0.5">
                      <div className="flex items-center gap-2 flex-wrap">
                        <p className="text-sm font-semibold text-[#0A2540]">{item.projectTitle}</p>
                        <ProjectTypeBadge type={item.projectType} />
                      </div>
                      {item.clientName && (
