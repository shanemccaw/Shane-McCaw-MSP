...
function ScoreBadge({ score }: { score: number | null | undefined }) {
  ...
  return (
    <span className={`inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-xs font-semibold ${colors[score] ?? "bg-gray-100 text-gray-500"}`}>
...
export default function M365IntelligencePage() {
  ...
  return (
    <div className="p-6 space-y-6 max-w-7xl mx-auto">
      {/* Header */}
      <div>
        <h1 className="text-2xl font-bold text-gray-900">M365 Intelligence</h1>
      ...
      {!isLoading && !isError && profiles.length > 0 && (
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
          <div className="bg-white rounded-xl border border-gray-200 p-4 col-span-2 sm:col-span-1 lg:col-span-2">
            ...
            <p className="text-xs text-gray-400 mt-0.5">across {profiles.length} client{profiles.length !== 1 ? "s" : ""}</p>
          </div>
          {([1, 2, 3, 4, 5] as const).map(s => {
            const colors: Record<number, string> = { 1: "text-red-600", 2: "text-orange-600", 3: "text-yellow-600", 4: "text-blue-600", 5: "text-green-600" };
            ...
            return (
              <button
                ...
                className={`bg-white rounded-xl border p-4 text-left transition-all hover:shadow-md ${scoreFilter === String(s) ? "border-[#0078D4] ring-1 ring-[#0078D4]" : "border-gray-200"}`}
              >
                <p className="text-xs font-medium text-gray-500 uppercase tracking-wide">{labels[s]}</p>
                <p className={`mt-1 text-2xl font-bold ${colors[s]}`}>{scoreGroups[s]}</p>
                <p className="text-xs text-gray-400 mt-0.5">Score {s}/5</p>
              </button>
            );
          })}
        </div>
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full pl-9 pr-4 py-2 text-sm border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#0078D4] focus:border-transparent"
          />
        </div>
        <select
          value={scoreFilter}
          onChange={e => setScoreFilter(e.target.value as ScoreFilter)}
          className="text-sm border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-[#0078D4] bg-white"
        >
      ...
      <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center py-16 gap-2">
            ...
            <p className="text-sm text-gray-500">Failed to load M365 profiles.</p>
          </div>
        ) : profiles.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-16 gap-2 text-center">
            ...
            <p className="text-sm font-medium text-gray-600">No M365 profiles yet</p>
                ...
                <tr>
                  <th className="text-left px-4 py-3">
                    <button
                      ...
                      className="flex items-center text-xs font-semibold text-gray-600 uppercase tracking-wide hover:text-gray-900"
                    >
                      Client
                ...
                {filtered.length === 0 ? (
                  ...
                  filtered.map(row => {
                    ...
                    const updatedLabel = row.updatedAt
                      ...
                      : "—";
                    return (
                      <tr key={row.clientId} className="hover:bg-gray-50 transition-colors">
                          ...
                          {skus.length === 0 ? (
                            ...
                            <div className="flex flex-wrap gap-1 max-w-[200px]">
                              {skus.slice(0, 3).map(sku => (
                                <span key={sku} className="inline-flex items-center px-1.5 py-0.5 rounded text-xs bg-gray-100 text-gray-700 font-mono">
                                  {sku}
                                </span>
                              ))}
                              {skus.length > 3 && (
