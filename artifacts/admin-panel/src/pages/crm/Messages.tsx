  ...
  return (
    <div className="p-4 sm:p-6 max-w-[1400px]">
      ...
      <div className="flex flex-col md:flex-row gap-4 md:h-[600px]">
        <div className={`md:w-64 flex-shrink-0 bg-card border border-border rounded-xl overflow-hidden flex flex-col ${selectedClient ? "hidden md:flex" : "flex"} max-h-52 md:max-h-none`}>
            ...
            <div className="flex-1 overflow-y-auto divide-y divide-border">
              {clients.map(c => (
                <button key={c.id} onClick={() => handleSelectClient(c)}
                  className={`w-full text-left px-4 py-3 hover:bg-accent transition-colors ${selectedClient?.id === c.id ? "bg-primary/5 border-l-2 border-primary" : ""}`}>
        ...
        <div className={`flex-1 bg-card border border-border rounded-xl overflow-hidden flex flex-col min-w-0 min-h-[400px] md:min-h-0 ${selectedClient ? "flex" : "hidden md:flex"}`}>
          {selectedClient && (
            <button onClick={() => setSelectedClient(null)}
              className="md:hidden flex items-center gap-1.5 px-4 py-2.5 border-b border-border text-xs font-semibold text-muted-foreground hover:bg-accent transition-colors text-left">
          ...
          {!selectedClient ? (
            ...
            <>
              <div className="px-5 py-3 border-b border-border flex-shrink-0">
                <p className="text-sm font-bold text-foreground">{selectedClient.name ?? selectedClient.email}</p>
                <p className="text-xs text-muted-foreground">{selectedClient.email}</p>
              </div>
              <div className="flex-1 overflow-y-auto px-5 py-4 space-y-3">
                {loadingMsgs ? (
                  <div className="flex items-center justify-center py-8"><div className="w-6 h-6 border-4 border-primary border-t-transparent rounded-full animate-spin" /></div>
                ) : messages.length === 0 ? (
                  ...
                  return (
                    <div key={msg.id} className={`flex ${fromAdmin ? "justify-end" : "justify-start"}`}>
                      <div className={`max-w-[75%] rounded-2xl px-4 py-2.5 ${fromAdmin ? "bg-primary text-white rounded-br-md" : "bg-accent border border-border text-foreground rounded-bl-md"}`}>
              ...
              <div className="px-5 py-3 border-t border-border flex-shrink-0">
                <form onSubmit={handleSend} className="flex gap-2">
                  <input value={body} onChange={e => setBody(e.target.value)}
                    ...
                    className="flex-1 border border-border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary bg-accent text-foreground" />
                  <button type="submit" disabled={!body.trim() || sending}
                    className="bg-primary text-white text-sm font-semibold px-4 py-2 rounded-lg hover:bg-primary/90 disabled:opacity-50 transition-colors flex items-center gap-1.5">
