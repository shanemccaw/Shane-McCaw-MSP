...
const TIER_COLORS: Record<Tier, string> = {
  ...
  Advanced: "bg-primary/100/15 text-blue-400",
        ...
        {messages.map((msg, i) =>
          msg.role === "assistant" ? (
            <div key={i} className="border-l-2 border-primary pl-3">
              <p className="text-[10px] font-bold text-primary uppercase tracking-wider mb-0.5">Q</p>
              <p className="text-sm text-foreground leading-relaxed">{msg.content}</p>
            </div>
          ) : (
            <div key={i} className="pl-4">
              <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider mb-0.5">A</p>
              <p className="text-sm text-foreground bg-accent rounded-lg px-3 py-2 leading-relaxed">{msg.content}</p>
            </div>
          )
        )}
      </div>
        ...
        <div className="flex-1 min-h-0 overflow-y-auto px-6 py-6 space-y-6">
          ...
          <div className="bg-accent rounded-xl p-4">
            <div className="flex items-center justify-between flex-wrap gap-3 mb-3">
              ...
              <span className="text-lg font-extrabold text-foreground">{lead.totalScore} <span className="text-sm font-normal text-muted-foreground">/ {totalMax}</span></span>
            </div>
            <div className="flex items-center gap-3">
              <span className={`text-xs font-bold px-2.5 py-1 rounded-full ${TIER_COLORS[lead.tier] ?? "bg-border/50 text-muted-foreground"}`}>
                {lead.tier}
              </span>
                ...
                {categoryKeys.map((key) => (
                  <div key={key}>
                    <div className="flex items-center justify-between mb-1">
                      <p className="text-xs text-foreground font-medium">{formatCategoryKey(key)}</p>
                    </div>
          ...
          {lead.analysisText && (lead.analysisText.whatThisMeans || lead.analysisText.whyThisFits || lead.analysisText.roiProjection) && (
            <div className="space-y-4">
              ...
              {lead.analysisText.whatThisMeans && (
                <div className="bg-accent rounded-xl p-4">
                  <p className="text-xs font-bold text-primary uppercase tracking-wider mb-1.5">What This Means</p>
                  <p className="text-sm text-foreground leading-relaxed">{lead.analysisText.whatThisMeans}</p>
                </div>
              )}
              {lead.analysisText.whyThisFits && (
                <div className="bg-accent rounded-xl p-4">
                  <p className="text-xs font-bold text-primary uppercase tracking-wider mb-1.5">Why This Fits</p>
                  <p className="text-sm text-foreground leading-relaxed">{lead.analysisText.whyThisFits}</p>
                </div>
              )}
              {lead.analysisText.roiProjection && (
                <div className="bg-accent rounded-xl p-4">
                  <p className="text-xs font-bold text-primary uppercase tracking-wider mb-1.5">ROI Projection</p>
                  <p className="text-sm text-foreground leading-relaxed">{lead.analysisText.roiProjection}</p>
                </div>
              )}
            </div>
          )}
        ...
        <div className="px-6 py-4 border-t border-border flex gap-3 flex-shrink-0">
          <button
            ...
            className={`flex-1 font-semibold rounded-lg py-2.5 text-sm transition-colors disabled:opacity-40 ${
              contacted
                ? "bg-accent text-foreground/90 hover:bg-border"
          ...
          <button
            onClick={onClose}
            className="px-4 border border-border rounded-lg text-sm font-medium text-muted-foreground hover:bg-accent transition-colors"
          >
            Close
          </button>
        </div>
    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
      ...
      <div className="relative bg-card border border-border rounded-xl shadow-2xl w-full max-w-md p-6">
        <div className="flex items-center gap-3 mb-4">
          ...
          <div>
            <h3 className="text-foreground font-bold text-base">Delete Quiz Lead</h3>
        ...
        <ul className="text-sm text-muted-foreground space-y-1.5 mb-4 pl-1">
          <li className="flex items-start gap-2">
            ...
            <span>Quiz lead — <strong className="text-foreground">{lead.name}</strong> ({lead.email})</span>
          </li>
          <li className="flex items-start gap-2">
            <span className="mt-1 w-1.5 h-1.5 rounded-full bg-red-400 flex-shrink-0" />
        ...
        <div className="flex gap-3">
          <button
            onClick={onClose}
            disabled={deleting}
            className="flex-1 px-4 py-2.5 border border-border rounded-lg text-sm font-medium text-muted-foreground hover:bg-accent transition-colors disabled:opacity-40"
          >
            Cancel
          </button>
          <button
...
export default function QuizLeadsPage() {
  ...
  return (
    <div className="p-4 sm:p-6 max-w-[1200px]">
      ...
      <div className="mb-6">
        <h1 className="text-xl font-bold text-foreground">Quiz Leads</h1>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        ...
        <StatCard label="Contacted" value={stats?.contacted ?? 0}
          icon={<svg viewBox="0 0 24 24" fill="none" className="w-5 h-5 text-primary" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>}
        />
        <StatCard label="Sample Report Downloads" value={downloadStats?.total ?? 0}
          icon={<svg viewBox="0 0 24 24" fill="none" className="w-5 h-5 text-primary" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4" /></svg>}
        />
      </div>
        ...
        <div className="bg-card border border-border rounded-xl p-5 mb-6">
          ...
          <div className="flex flex-wrap gap-2">
            {downloadStats.byQuizType.map(({ quizType, total: cnt }) => (
              <span key={quizType ?? "unknown"} className="inline-flex items-center gap-1.5 text-xs font-medium px-2.5 py-1 rounded-full bg-primary/10 text-primary">
      ...
      <div className="bg-card border border-border rounded-xl p-5 mb-6">
        <div className="flex items-center justify-between flex-wrap gap-4 mb-4">
          <div>
            <p className="text-sm font-bold text-foreground">Quick Wins Selector</p>
                ...
                <div key={slug} className="flex items-center gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between mb-1">
                      ...
                      <span className="text-xs font-extrabold text-primary ml-2 flex-shrink-0">{cnt}</span>
                    </div>
          ...
          <div className="flex flex-wrap gap-1.5">
            {TIER_OPTIONS.map(t => (
              <button key={t} onClick={() => { setTierFilter(t); setPage(1); void fetchLeads(1, t, contactedFilter, quizTypeFilter); }}
                className={`px-3 py-1.5 rounded-full text-xs font-semibold transition-colors ${tierFilter === t ? "bg-primary text-white" : "bg-accent text-muted-foreground hover:bg-primary/10 hover:text-primary"}`}>
                {t === "all" ? "All Tiers" : t}
              </button>
            ))}
          </div>
                  ...
                  {leads.map(lead => (
                    <tr key={lead.id} onClick={() => setSelectedLead(lead)}
                      ...
                      <td className="px-5 py-3.5">
                        ...
                        <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded-full mt-0.5 inline-block ${TIER_COLORS[lead.tier] ?? "bg-border/50 text-muted-foreground"}`}>
                          {lead.tier}
                        </span>
                      </td>
                      ...
                      <td className="px-5 py-3.5 font-semibold text-foreground">{lead.totalScore}</td>
                      <td className="px-5 py-3.5">
                        <span className={`text-xs font-semibold px-2.5 py-1 rounded-full ${TIER_COLORS[lead.tier] ?? "bg-border/50 text-muted-foreground"}`}>
                          {lead.tier}
                        </span>
                      </td>
                      ...
                      <td className="px-5 py-3.5">
                        {lead.contactedAt ? (
                          ...
                          <span className="text-xs font-semibold px-2.5 py-1 rounded-full bg-border/50 text-muted-foreground">No</span>
                        )}
                      </td>
              ...
              {leads.map(lead => (
                <div key={lead.id} className="px-4 py-3 flex items-center gap-3">
                  <div className="flex-1 min-w-0 cursor-pointer" onClick={() => setSelectedLead(lead)}>
                    ...
                    <p className="text-[10px] text-primary font-medium mt-0.5">{QUIZ_TYPE_LABELS[lead.quizType] ?? lead.quizType}</p>
                  </div>
                  <div className="flex flex-col items-end gap-1 flex-shrink-0">
                    <span className={`text-xs font-semibold px-2 py-0.5 rounded-full ${TIER_COLORS[lead.tier] ?? "bg-border/50 text-muted-foreground"}`}>{lead.tier}</span>
