...
export default function InvoiceDetailPage() {
      ...
      <div className="p-6 max-w-2xl">
        <div className="bg-red-500/10 border border-red-500/20 rounded-xl p-6 text-center">
          ...
          <button onClick={() => navigate("/crm/invoices")}
            className="text-sm text-primary hover:underline">← Back to Invoices</button>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="p-4 sm:p-6 max-w-[1100px] space-y-5">
        ...
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div className="flex flex-col gap-2">
            <div className="flex items-center gap-3">
              ...
              <span className={`text-[11px] font-bold px-2.5 py-1 rounded-full ${invoice.invoiceType === "retainer" ? "bg-purple-500/15 text-purple-400" : "bg-primary/15 text-primary"}`}>
        ...
        <Section title="Amount Breakdown">
          <div className="space-y-3">
            <div className="flex justify-between items-center">
              ...
              <span className="text-sm font-semibold text-foreground">{fmt(invoice.amount, invoice.currency)}</span>
            </div>
            {invoice.couponCode && (
              <div className="flex justify-between items-center">
            ...
            <div className="flex justify-between items-center pt-2 border-t border-border">
              ...
              <span className="text-lg font-bold text-foreground">{fmt(String(netAmount), invoice.currency)}</span>
            </div>
            <div className="pt-1">
          ...
          <div className="space-y-4">
            {invoice.client && (
              <div
                ...
                className="flex items-center gap-3 p-3 rounded-lg bg-accent hover:bg-accent transition-colors cursor-pointer border border-transparent hover:border-primary/30"
              >
                <div className="w-8 h-8 rounded-full bg-primary/20 flex items-center justify-center flex-shrink-0">
                  <svg className="w-4 h-4 text-primary" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}><path strokeLinecap="round" strokeLinejoin="round" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" /></svg>
                </div>
                <div className="min-w-0 flex-1">
                  <p className="text-sm font-semibold text-foreground truncate">{invoice.client.name ?? invoice.client.email}</p>
            ...
            {invoice.project && (
              ...
              >
                ...
                <div className="min-w-0 flex-1">
                  <p className="text-sm font-semibold text-foreground truncate">{invoice.project.title}</p>
            ...
            {invoice.contract && (
              <div className="flex items-center gap-3 p-3 rounded-lg bg-accent border border-border">
                ...
                <div>
                  <p className="text-sm font-semibold text-foreground">Contract {invoice.contract.contractVersion}</p>
          ...
          {invoice.sharepointFileUrl && (
            <div>
              ...
              <a href={invoice.sharepointFileUrl} target="_blank" rel="noreferrer"
                className="text-xs text-primary hover:underline truncate block">View in SharePoint ↗</a>
            </div>
          )}
        </div>
      </Section>
        ...
        <Section title="Invoice PDF">
          <div className="flex items-center gap-3">
            <button
              ...
              className="flex items-center gap-2 border border-border rounded-lg px-4 py-2 text-sm font-medium text-foreground hover:bg-accent transition-colors"
            >
      ...
      {/* AI Summary */}
      <div className="bg-card border border-border rounded-xl overflow-hidden">
        <div className="px-5 py-3 bg-accent border-b border-border flex items-center justify-between">
          ...
          <button
            ...
            disabled={aiLoading}
            className="flex items-center gap-2 text-xs font-semibold text-white bg-primary px-3 py-1.5 rounded-lg hover:bg-primary/90 disabled:opacity-60 transition-colors"
          >
            {aiLoading ? (
              <>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {invoice.invoiceType === "retainer" && aiSummary.churnProbability && (
                  <div className="bg-accent rounded-xl p-4 border border-border">
                    ...
                    <p className={`text-xl font-bold ${CHURN_COLORS[aiSummary.churnProbability] ?? "text-foreground"}`}>
