  ...
  return (
    <div className="border border-dashed border-purple-500/40 rounded-xl bg-purple-500/4 p-4 space-y-4">
      ...
      <div className="flex items-center gap-2">
        ...
        <span className="ml-auto text-[10px] text-[#484F58] bg-[#161B22] border border-[#21262D] px-2 py-0.5 rounded font-mono">DEV ONLY</span>
      </div>
      ...
      <div ref={dropdownRef} className="relative">
        <p className="text-[10px] font-semibold uppercase tracking-wider text-[#7D8590] mb-1.5">Assign to client</p>
        <button
          type="button"
          onClick={() => setShowDropdown(v => !v)}
          className="w-full flex items-center justify-between gap-2 px-3 py-2 bg-[#161B22] border border-[#30363D] rounded-lg text-xs text-left hover:border-purple-500/40 transition-colors"
        >
          <span className={selectedClient ? "text-[#E6EDF3]" : "text-[#484F58]"}>
            {selectedClient
              ? `${selectedClient.name ?? selectedClient.email}${selectedClient.company ? ` · ${selectedClient.company}` : ""}`
              : "No client / standalone"
            }
          </span>
          <svg className={`w-3.5 h-3.5 text-[#484F58] flex-shrink-0 transition-transform ${showDropdown ? "rotate-180" : ""}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
          ...
          <div className="absolute z-50 mt-1 w-full bg-[#161B22] border border-[#30363D] rounded-lg shadow-xl overflow-hidden">
            <div className="p-2 border-b border-[#21262D]">
              <input
                ...
                className="w-full bg-[#0D1117] border border-[#30363D] rounded px-2 py-1 text-xs text-[#E6EDF3] placeholder-[#484F58] focus:outline-none focus:border-purple-500/50"
              />
            </div>
            <div className="max-h-48 overflow-y-auto">
              <button
                ...
                className={`w-full text-left px-3 py-2 text-xs hover:bg-[#21262D] transition-colors ${!selectedClientId ? "text-purple-400 font-semibold" : "text-[#7D8590]"}`}
              >
                No client / standalone
              </button>
              {loadingClients && (
                <p className="px-3 py-2 text-xs text-[#484F58]">Loading…</p>
              )}
              {!loadingClients && filteredClients.length === 0 && searchTerm && (
                <p className="px-3 py-2 text-xs text-[#484F58]">No clients match "{searchTerm}"</p>
              )}
              {filteredClients.map(c => (
                <button
                  ...
                  className={`w-full text-left px-3 py-2 text-xs hover:bg-[#21262D] transition-colors ${selectedClientId === c.id ? "text-purple-400 font-semibold" : "text-[#E6EDF3]"}`}
                >
                  <span className="block truncate">{c.name ?? c.email}</span>
                  {c.company && <span className="block text-[10px] text-[#7D8590] truncate">{c.company}</span>}
                </button>
              ))}
            </div>
          </div>
      ...
      {/* Seed buttons */}
      <div>
        <p className="text-[10px] font-semibold uppercase tracking-wider text-[#7D8590] mb-2">Inject result</p>
      ...
      {/* Success log */}
      {lastResult && (
        <div className="space-y-2 px-3 py-3 bg-[#161B22] border border-[#30363D] rounded-lg">
          <div className="flex items-center gap-2">
          ...
          <p className="text-[11px] text-[#8B949E]">{lastResult.scriptName}</p>
          {lastResult.customerName && (
            <p className="text-[11px] text-[#7D8590]">Client: <span className="text-[#E6EDF3]">{lastResult.customerName}</span></p>
          )}
          {Object.keys(lastResult.scoreImpact).length > 0 && (
            <div className="flex flex-wrap gap-1.5 mt-1">
              {Object.entries(lastResult.scoreImpact).map(([k, v]) => (
                <span
                  key={k}
                  className={`text-[10px] font-bold px-1.5 py-0.5 rounded border ${v > 0 ? "text-green-400 border-green-500/30 bg-green-500/8" : v < 0 ? "text-red-400 border-red-500/30 bg-red-500/8" : "text-[#484F58] border-[#30363D]"}`}
                >
          ...
          {lastResult.findings.length > 0 && (
            <p className="text-[10px] text-[#484F58] italic">{lastResult.findings.length} AI finding{lastResult.findings.length !== 1 ? "s" : ""} generated</p>
          )}
        </div>
      )}
    </div>
