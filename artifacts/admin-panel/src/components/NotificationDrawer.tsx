...
interface NotificationDrawerProps {
  ...
  onPurchaseFlash?: (amount?: number, serviceName?: string) => void;
  onLeadFlash?: () => void;
}
      ...
      if (res.ok) {
        ...
        if (seenIdsRef.current === null) {
          ...
          if (newPurchases.length > 0) {
            ...
            const serviceMatch = bodyText.match(/^(.+?)\s*—\s*\$/);
            const serviceName = serviceMatch?.[1]?.trim() || undefined;
            onPurchaseFlash?.(amount, serviceName);
          }
