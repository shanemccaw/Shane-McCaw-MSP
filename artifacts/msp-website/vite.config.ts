...
// `previewPath = "/msp/"`, `services.env.BASE_PATH = "/msp/"`). In
// Staging/Production the whole platform is served from a single origin whose
// path router forwards `/msp/*` to this service — the same pattern
// `/portal/`, `/admin-panel/` and `/msp-console/` already use (see
// artifacts/portal/vite.config.ts for the full story of why a flat/absent
// BASE_PATH is wrong here). A flat/absent BASE_PATH defaults to this
// canonical base instead of failing config-eval outright.
const MSP_WEBSITE_CANONICAL_BASE = '/msp/';
...
export default defineConfig(async ({ command }) => {
  ...
  const basePath =
    !rawBasePath || rawBasePath === '/' ? MSP_WEBSITE_CANONICAL_BASE : rawBasePath;
