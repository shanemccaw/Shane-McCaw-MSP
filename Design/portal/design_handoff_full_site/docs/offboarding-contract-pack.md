...
- `GET /api/portal/dashboard` (read-only, for `customerStatus`) — already fully
  documented in `docs/portal/customer-home-and-timeline-contract-pack.md` §2; not
...
- **Report period** — `reports.period` (`index.ts:725`): `"weekly" | "monthly" |
  ...
  `docs/portal/documents-contract-pack.md` §3 already documents for `/portal/reports`.
...
- **`tenants.status` vs. Customer Home's `customerStatus`**: §5's `"inactive"`
  write is the exact field `docs/portal/customer-home-and-timeline-contract-pack.md` §2
...
- **`resolveCustomerUserIds` vs. `resolveSiblingUserIds`**: §5/§6 both call
  ...
  used by `docs/portal/documents-contract-pack.md` §1/§2) — same underlying bridge,
...
is fully covered by its own pack (`docs/portal/customer-home-and-timeline-contract-pack.md`)
