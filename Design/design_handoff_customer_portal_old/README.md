# Handoff: Customer Portal — Round One Build
...
One design file contains the whole thing: `Customer Portal Shell.dc.html`. Navigation is a single `active` state key — there is no router in the prototype. Every page listed below is a section inside that one file.
...
| File | Contents |
|---|---|
| `Customer Portal Shell.dc.html` | The entire portal — shell, all pages, all five systems, logic class |
