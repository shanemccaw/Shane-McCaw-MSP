...
// `crypto` -- this app deliberately keeps its runtime dependency list minimal (`pg`, and `sharp`
// as a real, explicit, Shane-approved exception for Git #3262's photo downscale -- see README
// "There is no framework and no build step") and webauthn/cbor were hand-implemented rather than
// reaching for a library for exactly the same reason. There is nothing exotic below; it is the
// same well-documented algorithm the `web-push` npm package implements, just without adding it.
//
