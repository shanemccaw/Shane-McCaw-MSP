...
| Apple Health bridge -- HealthKit is native-iOS-only, so a real Apple Shortcuts automation reads oxygen/heart-rate samples and POSTs them to a token-authed webhook; recent readings surface in Settings -> Connected, not a dedicated room | `health_metric_hook_tokens`/`health_metrics` tables (migration 072), `src/core/health-metrics.mjs`, `POST /hooks/health-metrics/:token` (`src/routes/health-metrics.mjs`), `GET/POST/DELETE /api/health-metrics/hook-tokens`, `GET /api/health-metrics`, `public/app.js` Settings -> Connected "Apple Health" section, `docs/apple-health-shortcuts-recipe.md` -- correlation with #3321's medication log deferred (#3322) |
