...
--
-- The Cost Engine chain broke on a check KEY, not on its inputs. Both inputs
-- were real and present:
--   * sku_price_reference has real pricing (ENTERPRISEPACK = 2300 cents).
--   * The tenant has real waste collected under `cost:unused-unassigned-licenses`
--     (unusedLicenseCount: 4, per-SKU breakdown incl. ENTERPRISEPACK: 1).
-- But every consumer (portal-assessment.ts's status.stats, the
-- licensing.wasteEstimateBreakdown dashboard metric, the CIO narrative) looked
-- ONLY at the hardcoded key `cost:license-waste-estimate` — the sole cost check
-- key referenced anywhere in the repo — and returned null when it had no row.
-- Monitor check keys are DATA (monitor_checks.key), so the code now resolves the
-- real waste-bearing cost check at query time (license-waste-source.ts) instead
-- of assuming one. Part A confirms which key the live catalog actually uses.
--
...
-- ════════════════════════════════════════════════════════════════════════════
-- PART A — Which cost check actually holds this tenant's per-SKU waste?
-- Expect: the row(s) the new resolver will pick up. `cost:license-waste-estimate`
-- may well not exist at all — that is the finding, not an error.
-- ════════════════════════════════════════════════════════════════════════════
...
-- Real collected rows per cost check for the test tenant (customer 4).
SELECT p.check_key,
       p.status,
       p.collected_at,
...
--      non-null, with `sourceCheckKey` naming the check it was priced from.
