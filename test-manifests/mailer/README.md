...
| 22 | **client-thread-reply** | ⚠ **no real trigger path** | DB row exists; the hardcoded `clientThreadReplyEmail()` was confirmed dead (never called, no `sendEmailFromTemplate`/`getEmailTemplateOrFallback`/workflow reference sent this slug) and deleted from `mailer.ts` (#3550). Only `admin-thread-reply` fires on status-report threads. Orphaned. |
