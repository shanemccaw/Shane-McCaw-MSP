...
- **PAY-TODAY discount offer**: When a client first views a presentation's payment step, the server records `firstVisitedAt` and generates a `PAY-TODAY` Stripe coupon (10% off, 72-hour expiry). The banner and discounted price are shown in `PaymentOptionsPanel.tsx` via a `useIsOfferLive` hook. The Stripe coupon is named `PAY-TODAY` — create it once in the Stripe Dashboard (Billing → Coupons → Create) with ID `PAY-TODAY`, percent-off 10, and duration "once". The server enforces expiry server-side before applying the coupon to any checkout session. The countdown in `PayTodayBanner.tsx` displays in `HH:MM:SS` format. The Agreement step is locked until payment is confirmed (status `"paid"` or `"signed"`); sidebar indicators and overview cards enforce this gate in addition to the footer.
