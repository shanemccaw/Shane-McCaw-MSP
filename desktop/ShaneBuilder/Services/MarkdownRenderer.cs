...
namespace ShaneBuilder.Services
{
    ...
    /// Git #2211 — ported verbatim from desktop/BuildConsole/Services/MarkdownRenderer.cs; the only
    /// change is BrushKeyMap below, which maps the Catppuccin-style flat brush names this file was
    /// written against onto ShaneBuilder's real dotted Brush.* resource keys (Themes/Colors.xaml)
    /// so the port doesn't require inventing a parallel resource dictionary.
    /// </summary>
    public static class MarkdownRenderer
    {
        ...
        // Git #2211 — see class remarks above.
        private static readonly Dictionary<string, string> BrushKeyMap = new()
        {
            ["PeachBrush"] = "Brush.LogSource.Build",
            ["BlueBrush"] = "Brush.Accent.Primary",
            ["TextBrush"] = "Brush.Text.Primary",
            ["Subtext1Brush"] = "Brush.Text.Muted",
            ["Subtext0Brush"] = "Brush.Text.Dim",
            ["Surface0Brush"] = "Brush.Bg.Chip",
            ["Surface1Brush"] = "Brush.Border.Default",
            ["MantleBrush"] = "Brush.Bg.Card",
            ["GreenBrush"] = "Brush.Alert.Success",
        };
        ...
        public static StackPanel Render(string? markdown, RenderOptions? options = null)
        {
            ...
            var getBrush = options.GetBrush ?? (key => (Brush)Application.Current.FindResource(
                BrushKeyMap.TryGetValue(key, out var mapped) ? mapped : key));
