...
/// <summary>Git #2331 — the screen/feature/build context captured onto a Test Pad note the moment
/// it's filed. Notes get taken mid test-pass, often minutes to hours before anyone reads them
/// back; without a stamp there's no way to tell later which screen the note was about, which
/// feature it belonged to, or which build was actually running when the behavior was observed.
/// <see cref="Screen"/> and <see cref="Feature"/> are free-text (whatever the caller's active-tab
/// resolver names them as — see <see cref="NoteContextStamper"/>) and may be <c>null</c> when
/// nothing is resolvable (e.g. no tab focused yet); <see cref="BuildNumber"/> is the GitHub issue
/// number of whichever build the queue shows as <c>running</c> at capture time, or <c>null</c> if
/// none is.</summary>
