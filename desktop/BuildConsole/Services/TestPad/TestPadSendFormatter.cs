...
/// <summary>Ported from ShaneBuilder (Git #2531, Feature: Test Pad #2530; originally Git #2337,
/// ShaneBuilder Feature: Test Pad #2326). Formats a batch of Test Pad notes into the single block
/// "Send to Claude" drops into the open chat's composer. Pure/stateless (takes notes in, returns
/// text out) so the pipeline step that follows (Claude architects, you approve, prompt + git
/// issue, Batter Up) can reuse the same block shape without this class knowing anything about
/// composers or windows. Every note's real stamp (screen/feature/build number, captured at file
/// time by <see cref="NoteContextStamper"/> via <see cref="TestPadService.AddNote"/>) rides along
/// — that's the whole point of stamping notes in the first place: so the note reads the same weeks
/// later as it did the moment it was taken.</summary>
