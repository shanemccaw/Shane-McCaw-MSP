...
{
    /// <summary>
    /// Splits a multi-statement SQL script on top-level semicolons only, skipping any
    /// that live inside single-quoted strings, double-quoted identifiers, dollar-quoted
    /// blocks, <c>--</c> line comments or <c>/* */</c> (nestable) block comments.
    /// Comment-/whitespace-only segments are dropped; a trailing statement with no final
    /// semicolon is still returned; each returned statement is trimmed and keeps its own
    /// terminating ';' when it had one.
    ///
    /// This is a C# port of the api-server's <c>sql-statement-splitter.ts</c>, kept
    /// behavior-identical to the server so a script splits the same way whether it runs
    /// through the HTTP path or the direct local <see cref="LocalSqlExecutor"/>. The SQL
    /// Runner UI (Controls/SqlRunnerView.xaml.cs) carries its own copy of the same port
    /// for its client-side auto-RETURNING rewrite — keep the two in sync if either moves.
    /// </summary>
