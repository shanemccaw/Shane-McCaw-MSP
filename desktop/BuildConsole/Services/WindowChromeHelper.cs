...
{
    /// <summary>
    /// Git #1006 — shared plumbing for the custom WindowStyle="None" title bar
    /// (originally built for MainWindow under Git #894, now applied to every
    /// top-level app window): dark immersive title bar + the WM_GETMINMAXINFO
    /// clamp that keeps a chromeless window's Maximize from covering the
    /// taskbar. Call <see cref="Setup"/> from each window's OnSourceInitialized.
    /// </summary>
