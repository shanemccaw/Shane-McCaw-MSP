...
// Git #2798 — real port of ShaneBuilder's landed Git Doctor service (Feature #2194,
// desktop/ShaneBuilder/Services/GitDoctorService.cs). Verbatim except the namespace:
// the class is fully self-contained (System.* only, shells real git against the repo
// this executable lives inside), so nothing about it is ShaneBuilder-specific and it
// ports 1:1 into BuildConsole. Every finding here comes from an actual git command
// against the real repo — no seeded/fabricated findings, per this project's "never
// invent data" rule. Two of the mockup's nine seeded checks are NOT implemented
// because they cannot be detected without a side effect that would itself mutate the
// repo (a real "pull would overwrite" only shows up mid-merge; a CRLF-only diff only
// shows up mid-checkout) — that is an honest gap, not an oversight, and callers should
// not assume full mockup parity.
    ...
    public async Task<IReadOnlyList<GitDoctorFinding>> RunChecksAsync()
    {
        ...
        if (RepoRoot == null)
        {
            findings.Add(new GitDoctorFinding("norepo", GitDoctorSeverity.High, "No git repository found", "n/a",
                "BuildConsole could not find a .git directory walking up from its own executable folder or the current working directory. Every other check needs a real repo to run against.",
