...
{
    /// <summary>
    /// The app's single, reusable, NON-BLOCKING notification engine — Shane: "a really nice
    /// smooth dialog engine that doesn't lock my UI ever." It replaces the scattered native
    /// <c>MessageBox.Show(...)</c> calls (blocking, modal, focus-stealing, with the system alert
    /// sound) with a soft dark-themed toast that stacks at the top-centre of the screen and
    /// auto-dismisses.
    ///
    /// The visual/behavioural pattern is generalized straight from <see cref="DeviceCodeWindow"/>
    /// (the app's proven always-on-top sign-in floaty): a borderless, transparent, Topmost window
    /// that never steals focus (SWP_NOACTIVATE + ShowActivated="False") and is never shown modally
    /// (<c>Show()</c>, never <c>ShowDialog()</c>). One shared <see cref="ToastHostWindow"/> hosts a
    /// vertical stack of <see cref="ToastCard"/>s so several notifications can be on screen at once.
    ///
    ...
    public static class ToastEngine
    {
        ...
        /// <summary>Warning / precondition-not-met toast (peach).</summary>
        ...
        private static void ShowCore(string title, string message, ToastKind kind, TimeSpan? duration, Action? onClick, bool persistent = false)
        {
            try
            {
                ...
                if (host == null)
                {
                    ...
                    // the app closes, hides when minimized) exactly like DeviceCodeWindow does — and
                    // so a lingering host can never keep the process alive at shutdown.
