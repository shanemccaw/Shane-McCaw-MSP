...
{
    /// <summary>Icon shown next to the message. Mirrors the handful of MessageBoxImage
    /// values every real call site in this app actually used (Error/Warning/Info/Question) —
    /// not a general-purpose icon set.</summary>
    ...
    /// <summary>
    /// Git #2175 — real custom-chrome dialog (WindowChromeHelper pattern, #2126/#2147
    /// tokens) replacing every native <c>MessageBox.Show</c> call site in ShaneBuilder.
    /// Static call-site API only — <see cref="Alert"/>, <see cref="Confirm"/>,
    /// <see cref="Input"/> — so future code has no reason to reach for MessageBox.Show.
    /// </summary>
