...
| `check-stranded-branches.mjs` | **Git #1447 Part 2** — sweeps every `agent/*` branch against main and reports which have commits main doesn't have ("stranded"). Deliberately separate from the worktree-lifecycle orphan sweep above — different question, different terminology. **Git #3025** — resolves each branch's real base independently rather than always diffing against `origin/main`: any ref matching the `<name>-base-<YYYY-MM-DD>` convention (e.g. `production-base-2026-09-05`, the New Site v2 / #2953 branch-split base) is picked up as a candidate alternate base, and whichever candidate gives the lowest ahead-count for a given branch is used and reported as `resolvedBaseUsed`. |
