...
| CR → drift attribution | `drift-change-attribution.ts:buildDriftScopeAttribution` — **Conditional Access only** (the one `DRIFT_CHECK_SPECS` entry with an `attribution` strategy), but **per setting**, not per category: the CR is walked to the Graph endpoint it actually wrote (`cr_executions` → `baseline_action_templates.endpoint` → `config_snapshot_resource_types`) and matched against the drifted setting's own resource/object/property through #2759's `config_change_scopes` bridge. A drift event carrying a real `crRef` reads as `approved`; nothing else does. #2819 replaced the predecessor here (`monitor-executor.ts:buildCaChangeRequestAttribution`, a 30-day category blanket that attributed EVERY drifted CA setting to any one qualifying CR). The domain boundary is deliberately not widened — `cr_pirs`'s own drift-rescan honesty rule (§8) restates the same CA-only limit | **CURRENT**, `#1497`/`#2819` |
