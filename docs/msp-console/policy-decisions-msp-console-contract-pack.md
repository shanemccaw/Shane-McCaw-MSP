...
**Re-verified by #3779** (2026-09-12) — real, full line-by-line re-audit against the current
backend, per the #1642 pattern, dispatched because #2589 (the original extraction) closed before
#3034 landed. Re-checking this pass found #3034/#3035 already correctly folded in by the
intervening #3351 regeneration (2026-09-09) — nothing further needed there. **The real gap this
pass found and closes: the MSP-side OU-assignment surface (`msp-active-directory.ts`, Git
#2148/#2524) was never documented in this pack at all**, despite being a real, live, tested
input into the Policy Engine's own OU-membership resolution (`policy-compliance-graph.ts` reads
manual assignments before falling back to department-match — see §0.2, new §3.9). Added this
pass; every other section re-verified unchanged from #3351.
...
that pass; only `docs/portal/policy-decisions-contract-pack.md` (the customer-portal pack, #1490/#1577)
...
**This is a separate module from `docs/portal/policy-decisions-contract-pack.md`** (the customer-portal
...
Plus, a real but genuinely tangential surface (added to this pack this pass, §3.9): **6 more
routes** in `artifacts/api-server/src/routes/msp-active-directory.ts` (Git #2148/#2524) — the
MSP-operator-gated manual OU-assignment CRUD + request-resolution flow that
`policy-compliance-graph.ts` reads before falling back to department-match membership guessing.
**16 routes across the module's real surface area**, 10 of them this pack's own core two-object
scope.
...
`docs/portal/policy-decisions-contract-pack.md` pack's own §8 "one row, `RBD-2026-575`" reference is
...
| `/api/msp/active-directory/ou/:id/assignments` | GET/POST | `msp-active-directory.ts:90-230` | `policy-compliance-graph.ts` reads the underlying table (§3.9) | live, staged for #2591 — real test coverage, added to this pack this pass (#3779) |
| `/api/msp/active-directory/ou-assignments/:id` | PATCH/DELETE | `msp-active-directory.ts:234-365` | same | live, staged for #2591 |
| `/api/msp/active-directory/ou-assignment-requests[/:id]` | GET/PATCH | `msp-active-directory.ts:372-520` | none (write-side of #2524's own request flow) | live, staged for #2591 |
...
### 3.9 OU manual assignment — `msp-active-directory.ts` (Git #2148/#2524) — added to this pack this pass (#3779)

**Not part of the Policy Decisions/Policy Engine module's own two objects — a real, tangential
input into how Policy Engine resolves OU membership**, documented here because
`policy-compliance-graph.ts` (§3.5's own OU-member resolution, real member of the OU) checks
manual assignments *first*, falling back to a department-match guess only when none exists
(`policy-compliance-graph.ts:91,154,199` — re-confirmed this pass). Without this section, a
reader of this pack would see `standing_policies.ou_id → active_directory_ous` as the whole
membership story; it is not — a real MSP operator can override it per-object.

Real history: #1952 built the underlying override (`active_directory_ou_assignments`,
admin-only, `admin-active-directory.ts`). #2148 filed the open question of whether that should
be MSP-operator/customer-self-service instead of PlatformAdmin-only; Shane's real decision
(2026-09-03, in #2148's own body) was **MSP-operator, through the MSP Portal — not customer
self-service.** `msp-active-directory.ts` is that MSP-staff-gated mirror, plus (#2524) the MSP
side of a customer-raised request-change loop. All 6 routes `requireCapability("ladder.msp-operator")`
+ `assertCustomerAccess` (MSPAdmin/PlatformAdmin clear the floor too) — same
ownership/staff-scoping discipline as every other route in this pack.

| Endpoint | Method | Route file:line | Notes |
|---|---|---|---|
| `/api/msp/active-directory/ou/:id/assignments` | GET | `msp-active-directory.ts:90-128` | Lists real manual assignments for one OU. A null-`tenantId` (platform-level) OU 404s here always — never a valid MSP-side read target (§ header, `:34-42`) |
| `/api/msp/active-directory/ou/:id/assignments` | POST | `msp-active-directory.ts:134-230` | Body `{customerId, objectUpn}`; Graph-verifies the UPN before writing; upserts on `(customerId, objectId)` — real unique constraint, confirmed live (`active_directory_ou_assignments_customer_object_uidx`) |
| `/api/msp/active-directory/ou-assignments/:id` | PATCH | `msp-active-directory.ts:234-305` | Body `{ouId}` — moves an existing assignment to a different OU scoped to the same customer |
| `/api/msp/active-directory/ou-assignments/:id` | DELETE | `msp-active-directory.ts:309-365` | Clears the override; membership reverts to the department-match guess |
| `/api/msp/active-directory/ou-assignment-requests` | GET | `msp-active-directory.ts:372-402` | Real customer-raised requests (`active_directory_ou_assignment_requests`, #2524), scoped to caller's MSP + staff book; optional `?status=` filter, unrecognised value ignored (display filter, not a write) |
| `/api/msp/active-directory/ou-assignment-requests/:id` | PATCH | `msp-active-directory.ts:415-520` | Body `{status: "approved"\|"rejected"\|"fulfilled", resolutionNote?}`. Approving/fulfilling a request with a real `requestedOuId` immediately applies the real assignment via the same Graph-verified upsert POST above uses — "approved" always means "actually true," never a status flip that leaves the real table stale (re-confirmed by direct read, `:457-501`) |

Every MSP-side write additionally refuses a null-`tenantId` OU as a target — `ou.tenantId !==
customerId` 400s (`:179-182`, `:279-282`) — because an OU with no tenant carries no `mspId` of
its own and could be shared across MSPs; only PlatformAdmin (via `admin-active-directory.ts`)
may touch that platform-wide case. **Real test coverage — `msp-active-directory.test.ts`, 24
cases** across all 6 routes (auth/role floor, cross-MSP 404-without-disclosure on both directions,
the null-tenant refusal, the happy-path Graph-verified assign/move/clear, and all three
request-resolution branches including the free-text-name-only case that leaves the assignment
table untouched — re-verified this pass via `grep "describe(\|it("`).

Live row counts, re-verified this pass alongside §9: `active_directory_ou_assignments` — **0**;
`active_directory_ou_assignment_requests` — **0**. Same honest-empty state as every other table
in this pack (§9) — the module is real and tested, nothing has been authored against it yet.

Schema, re-confirmed live this pass (`\d active_directory_ou_assignments` /
`\d active_directory_ou_assignment_requests`): both real tables, `active_directory_ou_assignments`
FK-cascades to `active_directory_ous`/`msps` and set-nulls its `assigned_by_user_id`;
`active_directory_ou_assignment_requests` set-nulls all three of its own OU/user FKs
(`current_ou_id`, `requested_ou_id`, `requested_by_user_id`, `resolved_by_user_id`) — no
CHECK constraint on `status` on either table (plain `text`, matching §5's general observation
about this module's tables), enforced only by `RESOLVABLE_STATUSES`/`ACTIVE_DIRECTORY_OU_ASSIGNMENT_REQUEST_STATUSES`
in application code.
...
| Manual OU assignment ↔ standing policy's real membership | `active_directory_ou_assignments.(customer_id, object_id)` | — | Not served by any route in this pack's own 10 | `policy-compliance-graph.ts` (§3.9) checks this table FIRST, department-match SECOND, when resolving who is really "in" the OU a standing policy targets — a real, live input into §3.5's evaluation that neither `standing_policies` nor `WireStandingPolicy` discloses on its own wire |
...
| `active_directory_ou_assignments` | **0** — verified this pass (§3.9, new to this pack) |
| `active_directory_ou_assignment_requests` | **0** — verified this pass (§3.9, new to this pack) |
...
**None filed this pass (#3779).** This pass re-verified #3351's regeneration line-by-line and
found it fully accurate — #3034 and #3035 were already correctly folded in, with no drift. The
one real gap found (§3.9's undocumented `msp-active-directory.ts` surface) was closed by
documenting it directly rather than filed as a separate finding — it is real, live, tested code
that was simply missing from this pack, not a product/backend gap needing new work.

**#3035's product question, restated per this issue's own dispatch:** #3779's own body asked
this pack to "flag #3035's unresolved product question... don't decide it, just document it's
genuinely open." Re-checking `gh issue view 3035` this pass confirms **it is not open** — #3035
was filed by the #2589 pack itself over a stale code-comment cross-reference (whether the
"no MSP-side create route" decision was tracked as its own issue), and #3351's regeneration
(§2.4 above) already confirms it closed with the route file's own header corrected to cite #3035
itself. There is no live "should MSP staff author a `policy_decisions` row on a customer's
behalf" question left open under #3035 — that door was deliberately never opened (§2.4's own
rationale: a misattributed `signedBy` is a trust decision, and no MSP-side create route exists,
by design). Restated here in case a future reader arrives via #3779 expecting an open item.

This regeneration (#3351) closes out the two real findings the prior
...
## Appendix — files read for #3779's re-verification pass (2026-09-12)

- `artifacts/api-server/src/routes/msp-active-directory.ts` (new to this pack — §3.9, full read,
  526 lines, Git #2148/#2524)
- `artifacts/api-server/src/routes/msp-active-directory.test.ts` (24 cases, all 6 routes)
- `artifacts/api-server/src/routes/admin-active-directory.ts` (cross-reference — the PlatformAdmin
  mirror this file extends, #1952's original build)
- `artifacts/api-server/src/lib/policy-compliance-graph.ts` (confirmed the real read order: manual
  assignment first, department-match fallback second — `:91,154,199`)
- `artifacts/api-server/src/routes/index.ts` (`:272` import, `:557` mount — confirmed live/mounted)
- Direct query, local Postgres (`shanemccawmsp`), this pass: `\d active_directory_ou_assignments`,
  `\d active_directory_ou_assignment_requests`, row counts for both (0/0)
- GitHub, re-checked this pass: #2148 (CLOSED, real decision recorded in its own body), #2524,
  #1952, #3779 itself, #3034/#3035 re-confirmed still CLOSED with no drift since #3351
