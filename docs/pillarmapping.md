...
**2026-09-18 addendum (#4573) — the count above is superseded.** The phantom-key backlog was re-audited one check at a time against the live catalog (202 active `monitor_checks`; none of the 65 remaining phantom keys exists in any status). Outcome: **6 remapped** to a real successor (`intune:non-compliant-devices` → `devices:compliant-vs-noncompliant`, `intune:unencrypted-devices` → `devices:encryption-status`, `collaboration:teams-channels` → `teams:channel-sprawl`, `compliance:guest-users` → `governance:guest-count`, `compliance:orphaned-teams` → `teams:ownerless-teams`, `copilot:overshare-exposure` → `copilot:data-exposure-risk`), **46 retired** to the `not_collected:` sentinel (no check measures the claim), and **13 left** in `CATALOG_DRIFT_BACKLOG` with a stated blocker each. Several of the ⚠ near-miss candidates named above turned out NOT to be successors (`devices:os-patch-compliance` counts every device, `devices:enrollment-status` is the opposite quantity, `cost:duplicate-assignments` is structurally always 0, `license:unused-assigned` has a malformed mapping, `security:secure-score-by-category` would render a single `unknown` bucket). The source of truth is now `lib/dashboard-registry/src/sourceKeyContract.ts` (`CATALOG_DRIFT_BACKLOG.blockers`) and `build-journal/4573-plan.md`; the family list in the paragraph above is history, not current state.

## 9. Portal surface — RETIRED (portal-v2 removed; the rebuild is #1485)

The prior edition's §9 was a page-by-page reverse map of the **portal-v2** customer portal, with a live-captured hero-tile backing map. **That surface no longer exists.** At commit `7a46c5d9`: `artifacts/msp-portal` itself doesn't exist any more (it was renamed to `artifacts/portal` in `f40438cdc`, #1921, and `7a46c5d9` postdates that rename), `artifacts/portal/src/pages/` contains **0 page files** (the `portal-v2-*` pages were NOT carried over in that rename), the `war-room-pillars` endpoint is gone, and `Design/portal/` (the live design source named by CLAUDE.md) carries **0 `.dc.html` exports**. `portal-v2` and `Design/design_handoff_customer_portal/` are retired per CLAUDE.md / Epic #1485 and are explicitly **not** a fallback. Carrying that reverse-map forward would document a dead UI as if it were a rebuild target, so it is deliberately dropped rather than presented as current.
