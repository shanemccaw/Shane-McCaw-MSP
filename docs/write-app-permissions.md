...
| 17 | `UserAuthenticationMethod.ReadWrite.All` | `mfa-enforcement-v1` step 1 (`action.require-security-info-reregistration`) forces MFA re-registration the way the Entra admin center's own button does: `GET /users/{id}/authentication/methods`, then `DELETE` each phone / Microsoft Authenticator / software OATH method individually (#1899), then RE-enumerate until two consecutive delayed reads corroborate the wipe (#2981 — a single read can land on an unconverged Entra replica and come back short, which used to be reported as success). Live-verified end-to-end against the sanctioned test user on #2840, and re-verified on #2981. | [List methods](https://learn.microsoft.com/en-us/graph/api/authentication-list-methods), [Delete phoneMethod](https://learn.microsoft.com/en-us/graph/api/phoneauthenticationmethod-delete) |
