...
| E | **MAILBOX-SEND app** (#4241) — used ONLY by the MSP mailbox connector (`sendMailViaGraphForMsp`, `lib/mailbox-send-app.ts`); `Mail.Send` application permission, admin-consented on the MSP's **own** tenant only, never on a customer tenant | **not yet created** (2026-09-15 — Shane's Azure AD action, DEV + PROD) | — | — | `MAILBOX_SEND_APP_CLIENT_ID` + either `MAILBOX_SEND_APP_CERT_PRIVATE_KEY` (single-line base64 PEM, #4156 format) + `MAILBOX_SEND_APP_CERT_THUMBPRINT` (preferred), or `MAILBOX_SEND_APP_CLIENT_SECRET`. Sends also require `msps.entra_tenant_id` (#4242) and a token whose `tid` equals it. |
