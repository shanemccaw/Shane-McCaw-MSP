...
**Blast radius:** single user. **Destructive?** No (reversible in effect, though `reversible = false` is recorded). **Executor-transportable:** yes. **Gated:** yes — the gate splices after step 2. **Operator variables:** `userId`. Steps: `action.require-security-info-reregistration` (**`GET /users/{{userId}}/authentication/methods`** — this row used to read `POST`, which is not a writable Graph v1.0 collection; #1899 replaced it with the real enumerate-then-`DELETE`-each-method fan-out, and the stored row is now `GET`, confirmed against the local DB on 2026-09-06. The `DELETE` half is live-verified on #2840, and #2981 added the re-enumeration that makes the step report a VERIFIED wipe instead of trusting one possibly-stale Entra read) → `action.enforce-per-user-mfa` (`PATCH /users/{{userId}}/authentication/requirements`, **gatedHere**). Note: this pack's step 2 is gate-flagged but there is no break-glass **account** created — the gate still fires and still generates + surfaces a secret (finding 6 / §5.4), a semantic quirk worth surfacing to the operator. It also carries a monitor `check_key` (`identity:mfa-registration`) on step 2's row, but the materializer treats `check_key` and `template_id` on the same row as the template (write) node.
...
2. **Portal mission-control instant remediation** — `artifacts/api-server/src/routes/portal-mission-control.ts` (`POST /portal/mission-control/remediate`), consumed by `artifacts/portal/src/components/BreakGlassPendingActionCard.tsx` (path corrected from `artifacts/msp-portal/src/components/BreakGlassPendingActionCard.tsx`; NOT carried over — retired in `f40438cdc`'s #1485 portal-v2 rebuild, this file does not exist in `artifacts/portal`). Fires a Config Pack for a testbed customer's accepted sales-offer, then surfaces the break-glass pending-secret via `GET /portal/break-glass/by-run/:runId`. **What it gets wrong / omits:** testbed-only (a non-testbed customer silently has zero remediable offers); it inherits every finding-1 / finding-4 limitation of the pack it runs.
...
1. ~~**The real payload of any successful write**~~ — **partially retired by #2840 (2026-09-06).** This entry read "not obtained; obtaining it requires a real mutation against the production testbed tenant, forbidden by this issue's safety rule," with the `403 Authorization_RequestDenied` refusal (§6.3) as the only live write outcome on record. #2840 provisioned a sanctioned, non-privileged, unlicensed test identity (`zz-test-graphwrite-01@mccawsoft2.onmicrosoft.com`, objectId `bdb21dc3-146a-4d97-a128-a2b8ff618d35`) precisely so that class of write could be fired safely — see [`testbed-destructive-write-test-user-2840.md`](testbed-destructive-write-test-user-2840.md). Real successful write outcomes now on record against it: `POST /users` → **201**, `POST /users/{id}/revokeSignInSessions` → **200**, `POST /users/{id}/authentication/phoneMethods` → **201**, `DELETE /users/{id}/authentication/phoneMethods/{methodId}` → **204**. Still not obtained: the `assignLicense` 200 body (the test identity is deliberately unlicensed) and a **completed pack run's** finalization payload — no pack has been run end-to-end against this account, only the individual Graph calls.
