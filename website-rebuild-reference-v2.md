...
- `text-tertiary` must never be used for text meant to be read (labels, captions, disclaimers) — confirmed to fail WCAG AA contrast at small sizes. Use `text-secondary` instead. This has regressed multiple times across separate tasks — any future task touching text color classes should check this rule explicitly, not assume prior fixes still hold.
